...
15. [FAQ](#faq)  
16. [Historia wydań (changelog)](#historia-wydań-changelog)  
...
## Historia wydań (changelog)

Pełne opisy wersji są na GitHubie:

- [Releases (najnowsze)](https://github.com/Adam7411/visionect_joan/releases)
- [Starsze wydania — strona 2](https://github.com/Adam7411/visionect_joan/releases?page=2)
- [Starsze wydania — strona 3](https://github.com/Adam7411/visionect_joan/releases?page=3)

<!-- Opcjonalny zrzut (np. lista wydań w przeglądarce):
![Releases na GitHubie](docs/images/PLACEHOLDER-github-releases.png)
-->

**Skrót (szczegóły zawsze na GitHubie):**

| Okres | Tematy |
|--------|--------|
| **3.8.x** | Tłumaczenia (UI HA, HTML na tablecie, powiadomienia), bateria i przełączniki, testy heurystyki rozmiaru ekranu, **analiza logów Ollama** (cyklicznie + przycisk na urządzenie), **watchdog VSS** / odzyskiwanie / opcjonalny URL sondy przed restartem sesji, usługa **Safe Config** (TCLV), poprawki UX baterii, m.in. 403/ip_ban, przecinek w liczbach, matplotlib. |
| **3.7.x** | Czujnik online, encje **select** (rotacja, sen), matplotlib na ARM, kalendarz wieloźródłowy, inteligentny układ pogody, poprawki encji, ścieżki HMAC, limit rozmiaru data-URL, RSS. |
| **3.6.x** | **`send_button_panel`**, **`send_keypad`**, `send_image_url`, `start_slideshow`, `clear_web_cache`, `set_session_options`, poprawki UI/tłumaczeń. |
| **3.5.x – 3.4.x** | Dolny pasek akcji, **`action_webhook_2_id`**, nazwy widoków, **`send_rss_feed`**, akcja „kliknij gdziekolwiek”, **Back button target** per urządzenie, webhooki, głębia bitowa i dithering. |

Wersja zainstalowana w Home Assistant to wartość z `manifest.json` w tym katalogu; może być nowsza niż ostatni **Release** na GitHubie do czasu publikacji tagu.
