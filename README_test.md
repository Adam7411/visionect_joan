...
6. [Predefiniowane widoki (Views)](#predefiniowane-widoki-views)  
   - [Menu opcji integracji](#menu-opcji-integracji)  
7. [Encje](#encje)  
...
### Ręcznie
1. Skopiuj katalog `visionect_joan` z repozytorium do `/config/custom_components/` (albo pobierz repozytorium jako ZIP i weź folder `custom_components/visionect_joan/`).
2. Ścieżka końcowa: `/config/custom_components/visionect_joan/`.
...
### Menu opcji integracji

**Ustawienia → Urządzenia i usługi → Visionect Joan → Konfiguruj** otwiera menu z pozycjami:

| Obszar | Zastosowanie |
|--------|----------------|
| **Predefiniowane widoki** | Dodawanie, edycja i usuwanie nazwanych URL (jak wyżej). |
| **Recovery page** | Token pod adres odzyskiwania / sondy hostowany w HA — wklejanie w VSS (HTML Backend) w razie potrzeby. |
| **Cache cleanup** | Maks. wiek plików (godziny) i interwał czyszczenia tymczasowych plików w `www` / `visionect_cache`. |
| **Tablet language** | Język generowanego HTML na tablecie (`auto` = jak w HA). |
| **Battery alerts** | Próg niskiej baterii (5 / 10 / 15 %), baner na tablecie, powiadomienia w HA (niski stan / pełna bateria). |
| **Ollama** | Opcjonalna, okresowa analiza logów VSS (adres, model, klucz API, interwał w minutach). |
...
| `button` | Force Refresh / Reboot / Clear Web Cache / Check health / analiza logów (Ollama) | Reboot i Clear cache domyślnie ukryte (kategorie diagnostic / config) |
...
| `visionect_joan.send_crypto` | Finanse | Tak | Dane z CryptoCompare + sparkline; identyfikatory jak slugi CoinGecko lub tickery (bez integracji crypto w HA) |
| `visionect_joan.set_session_options` | Render | N/A | Głębia / dithering |
| `visionect_joan.read_safe_device_config` | Utrzymanie | N/A | Odczyt bezpiecznych parametrów TCLV (powiadomienie + zdarzenie `command_result`) |
| `visionect_joan.apply_safe_device_config` | Utrzymanie | N/A | Zapis heart beat / ekrany systemowe / tryb dotyku (TCLV) |
| `visionect_joan.restore_safe_device_config` | Utrzymanie | N/A | Przywrócenie kopii z ostatniego udanego apply |
...
- `visionect_joan.send_crypto` – Panel cen: **CryptoCompare** dostarcza ceny i historię (bez klucza API). Lista monet używa **slugów w stylu CoinGecko** (wygoda); wewnętrznie mapowane na tickery API. Opcjonalnie Wstecz / auto-return jak przy innych treściach.  
- `visionect_joan.read_safe_device_config` / `apply_safe_device_config` / `restore_safe_device_config` – **Safe Config (TCLV)**: odczyt bezpiecznych parametrów, zapis (heart beat, ekrany systemowe, dotyk) albo przywrócenie kopii z ostatniego udanego **apply**. Typowy przebieg: **read** → weryfikacja w powiadomieniu → **apply**; **restore** cofa zmiany.  
...
### HAOS / Raspberry Pi — błąd przy starcie: `matplotlib`

Gdy **matplotlib** jest w `manifest.json` jako zależność pip, Home Assistant próbuje go doinstalować w kontenerze. Na części platform (**Python 3.14**, **aarch64**, ograniczone narzędzia budowania) instalacja może **paść** (Meson / budowa wheel), co **blokuje start integracji**.

**W obecnej wersji integracji matplotlib nie jest w manifeście.** Integracja się ładuje; **wykresy historii sensorów** i **układ pogody z wykresem** pomijają generowanie PNG, gdy matplotlib nie jest dostępny (przy `send_sensor_graph` na tablecie może pojawić się krótka informacja). Jeśli matplotlib jest w środowisku HA, wykresy działają bez dodatkowej konfiguracji.

### Joan 13 Pro — statyczny tekst „szarzeje”, a liczby/stany zostają czarne

> **Ryzyko uszkodzenia:** W dokumentacji Visionect jest mowa o tym, że **błędne zaawansowane parametry wyświetlacza (w tym VCOM) mogą uszkodzić panel e-papieru**. Poniższe kroki stosuj **wyłącznie**, gdy **wsparcie Visionect** potwierdziło, że dotyczą **Twojego** Joan 13 Pro. **Nie kopiuj** wartości na Joan 6, inne warianty Joan 13 ani inne urządzenia.

**Typowy obraz:** Fragmenty ekranu, które **rzadko się zmieniają**, po ok. **1–2 minutach** wyglądają na **szare / przygaszone**, a elementy **często odświeżane** (temperatury, stany encji, liczniki) zostają **czarne i czytelne**. To zwykle **kalibracja panelu po stronie VSS**, a nie wina Home Assistant, AppDaemon ani integracji.

**Jeśli support kazał ustawić parametry wyświetlacza** w Visionect Secure Signage (VSS):

1. Wybierz urządzenie → **Advanced** → **Device Configuration**.
2. Wejdź w **Display** (lewy górny róg).
3. Dla wyświetlacza **0**: **VCOM** = **2450**, **Display Type / ID** = **3759210982** *(przykładowa para podana przez wsparcie Visionect dla Joan 13 Pro — **Twój egzemplarz może wymagać innych wartości**)*.
4. **Write to device** i poczekaj na zakończenie zapisu.

Więcej kontekstu: [Visionect System Boards](https://docs.visionect.com/TechnicalDocumentation/Legacy/VisionectProducts/VisionectSystemBoards.html)

<!-- Miejsce na zrzut — wstaw własny plik, np. VSS → Device Configuration → Display:
![Ustawienia Display w VSS](docs/images/PLACEHOLDER-vss-display.png)
-->
