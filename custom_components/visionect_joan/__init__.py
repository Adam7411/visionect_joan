...
CONFIG_SCHEMA = vol.Schema({
    DOMAIN: vol.Schema({
        ...
        # Opcjonalna konfiguracja sprzątania plików medialnych (godziny)
...
# --- NOWY WSPÓLNY SCHEMAT DLA PRZYCISKU POWROTU ---
BACK_BUTTON_SCHEMA_EXTENSION = {
...
SERVICE_SEND_CALENDAR_SCHEMA = SERVICE_DEVICE_SCHEMA.extend({
    ...
    vol.Optional(ATTR_DAILY_VIEW_DURATION, default=5): vol.All(vol.Coerce(int), vol.Range(min=0, max=20)),
    **BACK_BUTTON_SCHEMA_EXTENSION,
})
...
SERVICE_SEND_STATUS_PANEL_SCHEMA = SERVICE_DEVICE_SCHEMA.extend({
    vol.Optional(ATTR_TITLE, default="Panel Stanu"): cv.string,
...
async def async_setup(hass: HomeAssistant, config: ConfigType) -> bool:
    ...
    # Uruchom cykliczne sprzątanie plików z www (migawki/wykresy)
    _schedule_media_cleanup(hass)
    return True

# --- Pomocnicze: czyszczenie plików mediów (migawki/wykresy) ---
...
# --- NOWA FUNKCJA POMOCNICZA DO WSTRZYKIWANIA PRZYCISKU ---
def _add_interactive_layer_to_url(
    ...
    """Dekoduje URL data:text/html, wstrzykuje przycisk powrotu i/lub kliknięcie ekranu, i koduje z powrotem.
       Uodpornione na brak <head>/<body> oraz wielkość liter; nie nadpisuje istniejących onclick."""
    if not (add_button or click_anywhere) or not back_url:
        return data_url
    ...
    try:
        ...
        # Zapewnij minimalny szkielet HTML, jeśli brakuje
        ...
        # Wstrzyknięcie <style> do <head>
        if re.search(r"</style>", decoded_html, re.IGNORECASE):
            decoded_html = re.sub(r"</style>", button_css + "</style>", decoded_html, count=1, flags=re.IGNORECASE)
        else:
            decoded_html = re.sub(r"</head>", f"<style>{button_css}</style></head>", decoded_html, count=1, flags=re.IGNORECASE)

        # Wstrzyknięcie przycisku (zatrzymuje propagację)
        if add_button:
            button_html = f'<a href="{back_url}" class="visionect-back-button" onclick="event.stopPropagation();">&#x2190;</a>'
            # wstaw tuż po otwarciu body
            decoded_html = re.sub(r"(<body\b[^>]*>)", r"\\1" + button_html, decoded_html, count=1, flags=re.IGNORECASE)

        # Dodanie listenera zamiast nadpisywać onclick atrybut
        if click_anywhere:
            click_script = f"""
            <script>
            (function() {{
                ...
                document.addEventListener('DOMContentLoaded', function() {{
                    // Nie nadpisuj istniejących onclick – dodaj listener
            ...
            """
            decoded_html = re.sub(r"</body>", click_script + "</body>", decoded_html, count=1, flags=re.IGNORECASE)

        return f"{header},{urllib.parse.quote(decoded_html, safe='')}"
    except Exception as e:
        _LOGGER.error(f"Nie udało się dodać przycisku powrotu do URL: {e}")
        return data_url
...
async def _get_icon_filename_for_entity(state: State) -> str:
    ...
    if device_class == "window": return "door-open.svg" if is_on_state else "window-closed.svg"
    ...
    if "okno" in entity_id: return "door-open.svg" if is_on_state else "window-closed.svg"
...
async def async_get_icon_as_base64(hass: HomeAssistant, icon_name: str) -> str:
    ...
    if not icon_file.is_file():
        _LOGGER.warning(f"Nie znaleziono pliku ikony: {icon_file}, używam domyślnej.")
        default_icon_file = Path(__file__).parent / "svg" / "default.svg"
        if not default_icon_file.is_file():
            _LOGGER.error("Brak nawet domyślnej ikony (default.svg)!")
            return ""
...
# --- Lokalizacja dat (PL/EN) bez zależności od locale systemowego ---
...
async def create_status_panel_url(hass: HomeAssistant, title: str, entity_ids: list[str]) -> str:
    STATE_TRANSLATIONS = { "on": "Włączone", "off": "Wyłączone", "open": "Otwarte", "closed": "Zamknięte", "home": "w Domu", "not_home": "Poza domem", "locked": "Zamknięte", "unlocked": "Otwarte", "unavailable": "Niedostępny", "unknown": "Nieznany", }
    style_css = """ body { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif; background-color: white; color: black; margin: 0; padding: 30px; box-sizing: border-box; } .header { text-align: center; font-size: 2.8em; font-weight: bold; border-bottom: 3px solid black; padding-bottom: 20px; margin-bottom: 25px; } .grid { display: grid; grid-template-columns: auto 1fr auto; gap: 20px 15px; align-items: center; font-size: 1.8em; } .icon { width: 1.5em; height: 1.5em; } .name { text-align: left; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; } .state { text-align: right; font-weight: bold; } .state-active { color: #d32f2f; } """
    items_html = ""
    for entity_id in entity_ids:
        ...
        display_state_val = STATE_TRANSLATIONS.get(display_state_val.lower(), display_state_val)
...
def create_text_message_url(message: str, text_color: str = "black", background_color: str = "white", text_size: str = "28px", text_align: str = "center", font_family_key: str = "Nowoczesna", font_weight: str = "normal", layout: str = "text_only", image_url: str = None, image_zoom: int = 100, image_rotation: int = 0) -> str:
    ...
    if layout == "image_only":
        style_css = f"""
            body {{ margin: 0; padding: 0; width: 100vw; height: 100vh; background-color: {background_color}; display: flex; align-items: center; justify-content: center; }}
        ...
        style_css = f"""
            {font_import_rule}
            body {{ font-family: {font_family_css}; color: {text_color}; background-color: {background_color}; margin: 0; padding: 0; width: 100vw; height: 100vh; display: flex; align-items: center; justify-content: center; text-align: {text_align}; box-sizing: border-box; -webkit-font-smoothing: none; font-smooth: never; }}
            .container {{ display: flex; flex-direction: {flex_direction}; align-items: center; justify-content: center; width: 90%; height: 90%; gap: 20px; }}
            .text-container {{ font-size: {text_size}; font-weight: {font_weight}; line-height: 1.5; word-wrap: break-word; flex-shrink: 1; }}
            .image-container {{ flex-shrink: 0; max-width: {'40%' if layout in ['image_left', 'image_right'] else '90%'}; max-height: {'90%' if layout in ['image_left', 'image_right'] else '40%'}; display: flex; align-items: center; justify-content: center;}}
        ...
        if not day_date:
            ...
            for event_date, day_events in events_by_day.items():
                ...
                for event in day_events:
                    html_body_content += _generate_event_html(event)
        else:
            for event in sorted_events:
                html_body_content += _generate_event_html(event)
...
def _generate_event_html(event: dict) -> str:
    """Generuje kod HTML dla pojedynczego wydarzenia z dodatkowymi szczegółami."""
...
def create_monthly_calendar_url(year: int, month: int, events: list, daily_view_duration_seconds: int = 5, lang: str = "en") -> str:
    style_css = """
        body { font-family: sans-serif; background-color: white; color: black; margin: 0; padding: 25px; box-sizing: border-box; }
...
def _generate_graph_image(
    ...
    """Generuje obraz wykresu dla podanych encji, zoptymalizowany dla orientacji ekranu."""
    try:
        import matplotlib
        matplotlib.use("Agg")
        ...
        _LOGGER.error("Biblioteka 'matplotlib' nie jest zainstalowana.")
        return None

    if not history_data:
        _LOGGER.warning(f"Nie przekazano danych historycznych dla encji: {entity_ids}")
        return None
    ...
    if not has_data:
        _LOGGER.warning("Po przefiltrowaniu nie znaleziono żadnych prawidłowych danych do narysowania wykresu.")
        plt.close(fig)
        return None
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    async def handle_send_text(call: ServiceCall):
        ...
        final_url = _add_interactive_layer_to_url(
            content_url, back_url, call.data[ATTR_ADD_BACK_BUTTON], call.data[ATTR_CLICK_ANYWHERE_TO_RETURN]
        )

        for uuid_val in uuids:
            status = "failure"
            try:
                if await api.async_set_device_url(uuid_val, final_url): status = "success"; _LOGGER.info(f"Tekst pomyślnie wysłany do {uuid_val}")
    ...
    async def handle_send_qr_code(call: ServiceCall):
        ...
        for uuid_val in uuids:
            ...
            status = "failure"
            try:
                if await api.async_set_device_url(uuid_val, final_url): status = "success"; _LOGGER.info(f"Kod QR pomyślnie wysłany do {uuid_val}")
    ...
    async def handle_send_calendar(call: ServiceCall):
        ...
        content_url = create_monthly_calendar_url(now.year, now.month, events, daily_view_duration, lang=lang) if style == "monthly_grid" else create_calendar_url(events, style, lang=lang)
        main_menu_url = hass.data[DOMAIN].get("main_menu_url")
        back_url = call.data.get(ATTR_BACK_BUTTON_URL) or main_menu_url
        final_url = _add_interactive_layer_to_url(
            content_url, back_url, call.data[ATTR_ADD_BACK_BUTTON], call.data[ATTR_CLICK_ANYWHERE_TO_RETURN]
        ...
        for uuid_val in uuids:
            ...
            status = "failure"
            try:
                if await api.async_set_device_url(uuid_val, final_url): status = "success"; _LOGGER.info(f"Kalendarz wysłany do {uuid_val}")
    ...
    async def handle_send_weather(call: ServiceCall):
        ...
        for uuid_val in uuids:
            ...
            if not weather_state: _LOGGER.error(f"Nie znaleziono encji pogody: {weather_entity_id}"); continue
            ...
            content_url = await create_weather_url(hass, weather_state, daily_forecast, hourly_forecast, layout, orientation)
            final_url = _add_interactive_layer_to_url(
                content_url, back_url, call.data[ATTR_ADD_BACK_BUTTON], call.data[ATTR_CLICK_ANYWHERE_TO_RETURN]
            )
            ...
            status = "failure"
            try:
                if await api.async_set_device_url(uuid_val, final_url): status = "success"; _LOGGER.info(f"Pogoda wysłana do {uuid_val}")
    ...
    async def handle_send_energy_panel(call: ServiceCall):
        ...
        for uuid_val in uuids:
            ...
            content_url = await create_energy_panel_url(hass, entity_states, orientation)
            final_url = _add_interactive_layer_to_url(
                content_url, back_url, call.data[ATTR_ADD_BACK_BUTTON], call.data[ATTR_CLICK_ANYWHERE_TO_RETURN]
            )
            ...
            status = "failure"
            try:
                if await api.async_set_device_url(uuid_val, final_url): status = "success"; _LOGGER.info(f"Panel energetyczny wysłany do {uuid_val}")
    ...
    async def handle_send_todo_list(call: ServiceCall):
        ...
        content_url = await create_todo_list_url(hass, title, items)
        main_menu_url = hass.data[DOMAIN].get("main_menu_url")
        back_url = call.data.get(ATTR_BACK_BUTTON_URL) or main_menu_url
        final_url = _add_interactive_layer_to_url(
            content_url, back_url, call.data[ATTR_ADD_BACK_BUTTON], call.data[ATTR_CLICK_ANYWHERE_TO_RETURN]
        ...
        for uuid_val in uuids:
            ...
            status = "failure"
            try:
                if await api.async_set_device_url(uuid_val, final_url): status = "success"; _LOGGER.info(f"Lista zadań wysłana do {uuid_val}")
    ...
    async def handle_send_status_panel(call: ServiceCall):
        """Obsługuje wysyłanie widoku panelu statusu."""
        uuids = await get_uuids_from_call(call)
        title = call.data[ATTR_TITLE]
        entity_ids = call.data[ATTR_ENTITIES]
        
        content_url = await create_status_panel_url(hass, title, entity_ids)
        main_menu_url = hass.data[DOMAIN].get("main_menu_url")
        back_url = call.data.get(ATTR_BACK_BUTTON_URL) or main_menu_url
        final_url = _add_interactive_layer_to_url(
            content_url, back_url, call.data[ATTR_ADD_BACK_BUTTON], call.data[ATTR_CLICK_ANYWHERE_TO_RETURN]
        ...
        for uuid_val in uuids:
            _LOGGER.info(f"Wysyłanie panelu statusu do {uuid_val}.")
            status = "failure"
            try:
                if await api.async_set_device_url(uuid_val, final_url):
                    status = "success"
                    _LOGGER.info(f"Panel statusu wysłany do {uuid_val}")
            except Exception as e:
                _LOGGER.error(f"Błąd wysyłania panelu statusu do {uuid_val}: {e}")
            
            hass.bus.async_fire(EVENT_COMMAND_RESULT, {
                "uuid": uuid_val, 
                "service": SERVICE_SEND_STATUS_PANEL, 
                "status": status
            })

    async def handle_send_sensor_graph(call: ServiceCall):
        """Obsługuje wysyłanie wykresu na podstawie historii sensorów."""
        uuids = await get_uuids_from_call(call)
        entity_ids = call.data[ATTR_ENTITIES]
        ...
        for device_uuid in uuids:
            ...
            try:
                image_bytes = await hass.async_add_executor_job(
                    _generate_graph_image, hass, history_data, entity_ids, graph_type, show_points, orientation
                )
            except Exception as e:
                _LOGGER.error(f"Nie udało się wygenerować obrazu wykresu dla {device_uuid}: {e}")
                image_bytes = None

            if not image_bytes:
                # Fallback: pokaż stronę HTML z komunikatem zamiast niczego
                lang = _get_lang(hass)
                ...
                _LOGGER.info(f"Wysyłanie komunikatu o braku danych do {device_uuid}.")
                status = "failure"
                try:
                    if await api.async_set_device_url(device_uuid, final_url):
                        status = "success"
                except Exception as e:
                    _LOGGER.error(f"Błąd podczas wysyłania komunikatu do {device_uuid}: {e}")
                hass.bus.async_fire(EVENT_COMMAND_RESULT, {
                    "uuid": device_uuid,
                    "service": SERVICE_SEND_SENSOR_GRAPH,
                    "status": status,
                    "params": {"entities": entity_ids}
                })
                continue
            ...
            _LOGGER.info(f"Wysyłanie wykresu z {entity_ids} do {device_uuid}.")
            status = "failure"
            try:
                if await api.async_set_device_url(device_uuid, final_url):
                    status = "success"
            except Exception as e:
                _LOGGER.error(f"Błąd podczas wysyłania wykresu do {device_uuid}: {e}")
