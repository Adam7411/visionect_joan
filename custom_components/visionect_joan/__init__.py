...
def _normalize_refresh_profile(value: str | None) -> str:
    """Normalize refresh profile and fallback to normal."""
    profile = str(value or REFRESH_PROFILE_NORMAL).strip().lower()
    return profile if profile in REFRESH_PROFILE_CHOICES else REFRESH_PROFILE_NORMAL
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    if not await api.async_test_authentication():
        ...
        return False
    api.set_battery_guard_interval(entry.options.get(CONF_BATTERY_GUARD_INTERVAL_SEC, 30))
