...
# UPDATED: No default title so user can omit it to hide header
SERVICE_SEND_BUTTON_PANEL_SCHEMA = SERVICE_DEVICE_SCHEMA.extend({
    vol.Optional("title"): vol.Any(None, cv.string),
    vol.Optional("button_1_webhook_id"): cv.string,
    vol.Optional("button_1_name"): cv.string,
    vol.Optional("button_1_icon"): cv.string,
    vol.Optional("button_2_webhook_id"): cv.string,
    vol.Optional("button_2_name"): cv.string,
    vol.Optional("button_2_icon"): cv.string,
    vol.Optional("button_3_webhook_id"): cv.string,
    vol.Optional("button_3_name"): cv.string,
    vol.Optional("button_3_icon"): cv.string,
    vol.Optional("button_4_webhook_id"): cv.string,
    vol.Optional("button_4_name"): cv.string,
    vol.Optional("button_4_icon"): cv.string,
    vol.Optional("button_5_webhook_id"): cv.string,
    vol.Optional("button_5_name"): cv.string,
    vol.Optional("button_5_icon"): cv.string,
    vol.Optional("button_6_webhook_id"): cv.string,
    vol.Optional("button_6_name"): cv.string,
    vol.Optional("button_6_icon"): cv.string,
    vol.Optional("button_7_webhook_id"): cv.string,
    vol.Optional("button_7_name"): cv.string,
    vol.Optional("button_7_icon"): cv.string,
    vol.Optional("button_8_webhook_id"): cv.string,
    vol.Optional("button_8_name"): cv.string,
    vol.Optional("button_8_icon"): cv.string,
    vol.Optional("button_9_webhook_id"): cv.string,
    vol.Optional("button_9_name"): cv.string,
    vol.Optional("button_9_icon"): cv.string,
    vol.Optional("button_10_webhook_id"): cv.string,
    vol.Optional("button_10_name"): cv.string,
    vol.Optional("button_10_icon"): cv.string,
    vol.Optional("button_11_webhook_id"): cv.string,
    vol.Optional("button_11_name"): cv.string,
    vol.Optional("button_11_icon"): cv.string,
    vol.Optional("button_12_webhook_id"): cv.string,
    vol.Optional("button_12_name"): cv.string,
    vol.Optional("button_12_icon"): cv.string,
    vol.Optional("small_screen_optimized", default=False): cv.boolean,
    vol.Optional("add_back_button", default=False): cv.boolean,
    vol.Optional("click_anywhere_to_return", default=False): cv.boolean,
    vol.Optional("click_anywhere_to_action", default=False): cv.boolean,
    vol.Optional("back_button_url"): cv.string,
    vol.Optional("action_webhook_id"): cv.string,
    vol.Optional("action_webhook_2_id"): cv.string,
})
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    async def handle_send_weather(call: ServiceCall):
        ...
        for device_uuid in uuids:
            ...
            content_url = await create_weather_url(
                hass, weather_state, daily_forecast, hourly_forecast,
                layout, orientation, lang, small_screen
            )
            back_url = _get_back_url_for_uuid(device_uuid, call.data)
            add_back = _effective_add_back_button(call, back_url)
            final_url = await _add_interactive_layer_to_url(
    ...
    async def handle_send_sensor_graph(call: ServiceCall):
        ...
        history_data = await get_instance(hass).async_add_executor_job(
            partial(history.get_significant_states, hass, start_time, entity_ids=entity_ids, significant_changes_only=False)
        )
        www_dir = Path(hass.config.path("www"))
        await hass.async_add_executor_job(lambda: www_dir.mkdir(parents=True, exist_ok=True))
        ...
        for device_uuid in uuids:
            ...
            content_url = create_text_message_url(
                message="", layout="image_only",
                image_url=image_url, image_zoom=image_zoom, image_rotation=image_rotation
            )
            add_back = _effective_add_back_button(call, back_url)
            final_url = await _add_interactive_layer_to_url(
                hass,
...
(function(){{
  ...
  setInterval(next, Math.max(1, sec)*1000);
    ...
    async def handle_send_button_panel(call: ServiceCall):
        ...
        buttons = []
        for i in range(1, 13):
            webhook_id = call.data.get(f"button_{i}_webhook_id")
            if webhook_id:
                buttons.append({
                    "webhook_url": _get_webhook_url(webhook_id),
        ...
        if not buttons:
            _LOGGER.error("send_button_panel service called with no valid buttons (webhook_id is required).")
            return
        ...
        for device_uuid in uuids:
            ...
            try:
                ...
                _LOGGER.error(f"Error sending button panel to {device_uuid}: {e}")
