...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    async def async_update_data():
        try:
            ...
            for device_entry in devices_summary:
                ...
                # fetch_session=False: session data (URL, ReloadTimeout) rarely changes and is cached in api.py.
                # This avoids unnecessary /api/session/{uuid} calls every 15 minutes.
                device_details = await api.async_get_device_data(uuid_val, fetch_session=False)
    ...
    def _get_prefs() -> dict:
        return hass.data[DOMAIN][entry.entry_id].setdefault(
            "prefs", {"back_targets": {}, "screen_size_by_uuid": {}, "safe_last_applied_by_uuid": {}}
        )
    ...
    async def handle_diagnose_vss_session(call: ServiceCall):
        ...
        for device_uuid in uuids:
            ...
            coord_dev = coordinator.data.get(normalized_uuid, {}) if isinstance(coordinator.data, dict) else {}
