...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    async def handle_send_calendar(call: ServiceCall):
        ...
        # ✅ ZMIANA: Zbierz eventy z WSZYSTKICH kalendarzy
        all_events = []
        
        for cal_entity in calendar_entities:
            try:
                # Fetch events z tego kalendarza
                ...
                try:
                    ...
                    # Wyciągnij eventy z response
                    raw_events = events_response.get(cal_entity, {}).get("events", [])
                    
                    # ✅ PARSOWANIE DAT: Konwertuj stringi na datetime
                    for event in raw_events:
                        ...
                        if start_str:
                            try:
                                parsed_start = dt_util.parse_datetime(start_str)
                                # Upewnij się że data ma timezone
                        ...
                        if end_str:
                            try:
                                parsed_end = dt_util.parse_datetime(end_str)
                                # Upewnij się że data ma timezone
                        ...
                        # Dodaj tylko jeśli ma start
                        if 'start' in event and event['start'] is not None:
                            all_events.append(event)
                        
                    _LOGGER.debug(f"Fetched {len(raw_events)} events from {cal_entity}")
                ...
                _LOGGER.error(f"Error processing calendar {cal_entity}: {e}")
                continue
        ...
        _LOGGER.info(f"Total merged events: {len(all_events)} from {len(calendar_entities)} calendars")
        ...
        if display_style == "monthly_grid":
            ...
            content_url = create_monthly_calendar_url(
                year, month, all_events, lang=lang, small_screen=small_screen
            )
            ...
            if not weather_entity_id:
                ...
                content_url = create_calendar_url(all_events, style="modern", lang=lang)
            else:
            ...
            content_url = create_calendar_url(all_events, style=display_style, lang=lang)
        ...
        for device_uuid in uuids:
            ...
            if display_style == "weather_calendar": 
                if daily_forecast is not None:
                    ...
                    current_content_url = await create_weather_calendar_url(
                        ...
                        small_screen=small_screen, daily_forecast=daily_forecast,
            ...
            if current_content_url is None:
                 # Safety fallback
                 current_content_url = create_calendar_url(all_events, style="modern", lang=lang)
