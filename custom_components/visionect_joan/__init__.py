...
### KLUCZOWA ZMIANA NR 1: Poprawka schematu `device_id` ###
# Zmieniamy `cv.string` na `vol.Any(cv.string, [cv.string])`, aby akceptować
# zarówno pojedyncze urządzenie (jako string), jak i listę urządzeń.
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    async def handle_send_text(call: ServiceCall):
        ...
        for device_uuid in uuids:
            ...
            final_url = await _add_interactive_layer_to_url(
                ...
                back_url,
                add_back,
                call.data[ATTR_CLICK_ANYWHERE_TO_RETURN],
                call.data[ATTR_CLICK_ANYWHERE_TO_ACTION],
    ...
    # Helper to generate slideshow HTML
    def _build_slideshow_data_url(urls: list[str], seconds: int, loop: bool) -> str:
        # Do not add cache buster to data: URLs
        processed = [
