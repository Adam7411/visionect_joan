...
CONFIG_SCHEMA = vol.Schema({
    DOMAIN: vol.Schema({
        ...
        # Optional configuration for media file cleanup (hours)
...
# --- NEW COMMON SCHEMA FOR THE BACK BUTTON ---
...
async def async_setup(hass: HomeAssistant, config: ConfigType) -> bool:
    ...
    # Start periodic cleanup of media files (snapshots/graphs)
    _schedule_media_cleanup(hass)
    return True

# --- Helper: clean up media files (snapshots/graphs) ---
...
# --- NEW HELPER FUNCTION TO INJECT THE BUTTON ---
def _add_interactive_layer_to_url(
    ...
    """Decodes a data:text/html URL, injects a back button and/or screen click, and encodes it back.
       Resistant to missing <head>/<body> and case-insensitivity; does not overwrite existing onclick."""
    ...
    try:
        ...
        # Ensure a minimal HTML skeleton if it's missing
        ...
        if not re.search(r"<head\b", decoded_html, re.IGNORECASE):
            # FIX: Changed `\\1` to `\1` for correct backreference
        ...
        if click_anywhere:
            ...
            (function() {{
                ...
                document.addEventListener('DOMContentLoaded', function() {{
                    // Don't overwrite existing onclick - add a listener
        ...
        _LOGGER.error(f"Failed to add back button to URL: {e}")
        return data_url
...
# --- Date localization (PL/EN) without dependency on system locale ---
...
async def create_status_panel_url(hass: HomeAssistant, title: str, entity_ids: list[str], lang: str) -> str:
...
async def create_todo_list_url(hass: HomeAssistant, title: str, items: list, lang: str) -> str:
    """Asynchronously generates a data:text/html URL for a to-do list with dynamic font size."""
...
async def create_energy_panel_url(hass: HomeAssistant, states: dict[str, State | None], orientation: str, lang: str) -> str:
    """Asynchronously generates a data:text/html URL for the energy panel."""
...
def _generate_event_html(event: dict, no_title_text: str) -> str:
    """Generates HTML code for a single event with additional details."""
...
async def create_weather_url(
    ...
    """Asynchronously generates a data:text/html URL for the weather view."""
    ...
    no_daily_data_label = "Brak danych prognozy dziennej." if lang == "pl" else "No daily forecast data."
    no_hourly_data_label = "Brak prognozy godzinowej" if lang == "pl" else "No hourly forecast"
    ...
    if layout == "current_large":
        ...
        if hourly_forecast:
            for hour in hourly_forecast[:4]: 
             ...
             html_body += f'<div style="font-size: 1.5em;">{no_hourly_data_label}</div>'
        html_body += '</div>'
...
def _generate_graph_image(
    ...
    """Generates a graph image for the given entities, optimized for screen orientation."""
    try:
        import matplotlib
        matplotlib.use("Agg")
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    async def handle_set_url(call: ServiceCall):
        ...
        for uuid_val in uuids:
            status = "failure"
            try:
                if await api.async_set_device_url(uuid_val, url_with_buster) and await api.async_restart_session(uuid_val): status = "success"; _LOGGER.info(f"URL for {uuid_val} successfully changed to: {original_url}")
            except Exception as e: _LOGGER.error(f"Error while changing URL for {uuid_val}: {e}")
    ...
    async def handle_send_text(call: ServiceCall):
        ...
        for uuid_val in uuids:
            status = "failure"
            try:
                if await api.async_set_device_url(uuid_val, final_url): status = "success"; _LOGGER.info(f"Text successfully sent to {uuid_val}")
            except Exception as e: _LOGGER.error(f"Error sending text to {uuid_val}: {e}")
    ...
    async def handle_force_refresh(call: ServiceCall):
        ...
        for uuid_val in uuids:
            status = "failure"
            try:
                if await api.async_restart_session(uuid_val): status = "success"; _LOGGER.info(f"Session for {uuid_val} has been restarted")
            except Exception as e: _LOGGER.error(f"Error restarting session for {uuid_val}: {e}")
    ...
    async def handle_send_weather(call: ServiceCall):
        ...
        for uuid_val in uuids:
            ...
            content_url = await create_weather_url(hass, weather_state, daily_forecast, hourly_forecast, layout, orientation, lang)
            final_url = _add_interactive_layer_to_url(
                content_url, back_url, call.data[ATTR_ADD_BACK_BUTTON], call.data[ATTR_CLICK_ANYWHERE_TO_RETURN]
            )
            ...
            status = "failure"
            try:
                if await api.async_set_device_url(uuid_val, final_url): status = "success"; _LOGGER.info(f"Weather sent to {uuid_val}")
    ...
    async def handle_send_energy_panel(call: ServiceCall):
        ...
        for uuid_val in uuids:
            ...
            content_url = await create_energy_panel_url(hass, entity_states, orientation, lang)
            final_url = _add_interactive_layer_to_url(
                content_url, back_url, call.data[ATTR_ADD_BACK_BUTTON], call.data[ATTR_CLICK_ANYWHERE_TO_RETURN]
            )
            ...
            status = "failure"
            try:
                if await api.async_set_device_url(uuid_val, final_url): status = "success"; _LOGGER.info(f"Energy panel sent to {uuid_val}")
    ...
    async def handle_send_todo_list(call: ServiceCall):
        ...
        content_url = await create_todo_list_url(hass, title, items, lang)
        main_menu_url = hass.data[DOMAIN].get("main_menu_url")
        back_url = call.data.get(ATTR_BACK_BUTTON_URL) or main_menu_url
        final_url = _add_interactive_layer_to_url(
            content_url, back_url, call.data[ATTR_ADD_BACK_BUTTON], call.data[ATTR_CLICK_ANYWHERE_TO_RETURN]
        ...
        for uuid_val in uuids:
            ...
            status = "failure"
            try:
                if await api.async_set_device_url(uuid_val, final_url): status = "success"; _LOGGER.info(f"To-do list sent to {uuid_val}")
    ...
    async def handle_send_status_panel(call: ServiceCall):
        """Handles sending the status panel view."""
        uuids = await get_uuids_from_call(call)
        title = call.data[ATTR_TITLE]
        entity_ids = call.data[ATTR_ENTITIES]
        lang = _get_lang(hass)
        
        content_url = await create_status_panel_url(hass, title, entity_ids, lang)
        main_menu_url = hass.data[DOMAIN].get("main_menu_url")
        back_url = call.data.get(ATTR_BACK_BUTTON_URL) or main_menu_url
        final_url = _add_interactive_layer_to_url(
            content_url, back_url, call.data[ATTR_ADD_BACK_BUTTON], call.data[ATTR_CLICK_ANYWHERE_TO_RETURN]
        ...
        for uuid_val in uuids:
            ...
            try:
                if await api.async_set_device_url(uuid_val, final_url):
                    status = "success"
                    _LOGGER.info(f"Status panel sent to {uuid_val}")
            except Exception as e:
                _LOGGER.error(f"Error sending status panel to {uuid_val}: {e}")
    ...
    async def handle_send_sensor_graph(call: ServiceCall):
        """Handles sending a graph based on sensor history."""
        uuids = await get_uuids_from_call(call)
        entity_ids = call.data[ATTR_ENTITIES]
        ...
        for device_uuid in uuids:
            ...
            if not image_bytes:
                # Fallback: show an HTML page with a message instead of nothing
                lang = _get_lang(hass)
