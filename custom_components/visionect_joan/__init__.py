...
def _build_ollama_context_for_notification(
    ...
    *,
    ...
    suffix_part = "… (truncated)" if language == "en" else "… (skrócono)"
    trimmed: list[str] = []
...
async def _async_ollama_analyze_logs(
    ...
    *,
    ...
    if language == "en":
        prompt = (
            "You are a VSS diagnostics assistant. RESPOND ONLY IN ENGLISH.\n"
    ...
    # Normalize output style in a second pass to keep HA notification compact and Polish.
    if language == "en":
        rewrite_prompt = (
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
            ...
            for device_entry in devices_summary:
                ...
                try:
                    ...
                    if battery is not None:
                        ...
                        # Clear tablet alert and restore previous content at safer level
                        battery_tablet_clear_threshold = 20.0
                        ...
                        if batt_val >= 90.0 and is_charging:
                            if uuid_val not in guard_state["full_charge_notified"]:
                                ...
                                is_pl = str(notif_lang).startswith("pl")
                                full_msg = (
                                    f"Tablet **{device_name}** jest naładowany ({batt_val:.0f}%). Możesz odłączyć ładowarkę."
                                    if is_pl
                                    else f"Tablet **{device_name}** is fully charged ({batt_val:.0f}%). You can unplug the charger."
                                )
                                full_title = "Tablet naładowany (Visionect)" if is_pl else "Tablet Fully Charged (Visionect)"
                                try:
                                    async_create_persistent_notification(
                                        hass,
                                        full_msg,
    ...
    if ollama_enabled and ollama_url and ollama_model:
        ...
        ai_lang = "en" if entry.options.get(CONF_TABLET_LANGUAGE, "auto") == "en" else "pl"
        ai_title = "Visionect AI log analysis" if ai_lang == "en" else "Analiza AI logów Visionect"
        ...
        async def _async_ollama_tick(_now: datetime) -> None:
            ...
            try:
                ...
                try:
                    ...
                    msg = (
                        f"[Visionect API]\nCould not load device list: {api_err}"
                        if ai_lang == "en"
                        else f"[Visionect API]\nNie udało się pobrać listy urządzeń: {api_err}"
                    )
                    extra_sections.append(msg)
                ...
                if analysis:
                    sep = (
                        "--- What the AI analyzed (VSS logs if available + full device list from API) ---"
                        if ai_lang == "en"
                        else "--- Co analizowało AI (logi VSS, jeśli są + pełna lista urządzeń z API) ---"
                    )
                    ...
                    if not ai_warned["missing_logs"]:
                        async_create_persistent_notification(
                            hass,
                            (
                                "AI diagnostics: no data available for analysis (no logs and no API data)."
                                if ai_lang == "en"
                                else "AI diagnostyka: brak danych do analizy (logi niedostępne i API nie zwróciło danych)."
                            ),
                            title=ai_title,
                            notification_id=f"{DOMAIN}_ollama_analysis_{entry.entry_id}",
                        )
                ...
                async_create_persistent_notification(
                    hass,
                    (f"AI diagnostics error: {err}" if ai_lang == "en" else f"Błąd AI diagnostyki: {err}"),
                    title=ai_title,
                    notification_id=f"{DOMAIN}_ollama_analysis_{entry.entry_id}",
                )
            finally:
    ...
    def _format_safe_values(values: dict[str, str]) -> str:
        if not values:
            return "- (no supported safe fields detected)"
        lines = []
        for key, val in values.items():
            label = SAFE_FIELD_LABELS.get(key, key)
            display_val = str(val).strip()
            if display_val == "":
                display_val = "Brak odpowiedzi urządzenia"
            lines.append(f"- {label}: `{display_val}`")
        return "\n".join(lines)

    def _format_applied_values(values: dict[str, str], is_pl: bool) -> str:
        if not values:
            return "- " + ("Brak ustawionych pól" if is_pl else "No applied fields")
        ...
        for key, raw in values.items():
            ...
            if key == ATTR_HEART_BEAT_INTERVAL and val != "":
                lines.append(
                    f"- {label}: " + (f"ustawiono na `{val}` minuty" if is_pl else f"set to `{val}` minutes")
                )
            elif key == ATTR_SYSTEM_SCREENS and val != "":
                pretty = system_screens_map.get(val, val)
                lines.append(
                    f"- {label}: " + (f"ustawiono na `{pretty}`" if is_pl else f"set to `{pretty}`")
                )
                ...
                lines.append(
                    f"- {label}: " + (f"ustawiono na `{val}`" if is_pl else f"set to `{val}`")
                )
        return "\n".join(lines)
    ...
    def _format_safe_ids(ids: dict[str, int]) -> str:
        if not ids:
    ...
    def _format_diag_rows(rows: list[dict]) -> str:
        if not rows:
        ...
        for row in rows:
            name = row.get("name", "?")
            tid = row.get("id", "?")
            val = str(row.get("value", "")).strip() or "Brak odpowiedzi urządzenia"
            lines.append(f"- {name} (ID `{tid}`): `{val}`")
        return "\n".join(lines)

    def _format_verify_report(requested: dict[str, str], confirmed: dict[str, str], is_pl: bool) -> str:
        if not requested:
            return "- " + ("Brak pól do weryfikacji" if is_pl else "No fields to verify")

        confirmed_ok = 0
        different = 0
        no_response = 0
        for key, req_val in requested.items():
            conf_val = str(confirmed.get(key, "")).strip()
            req_val_s = str(req_val).strip()
            if conf_val == "":
                no_response += 1
            elif conf_val == req_val_s:
                confirmed_ok += 1
            else:
                different += 1
        ...
        if is_pl:
            return "\n".join(
                [
                f"- Potwierdzone: `{confirmed_ok}`",
                f"- Różne od żądanych: `{different}`",
                f"- Brak odpowiedzi urządzenia: `{no_response}`",
                ]
            )

        return "\n".join(
            [
                f"- Confirmed: `{confirmed_ok}`",
                f"- Different from requested: `{different}`",
                f"- No device response: `{no_response}`",
            ]
        )

    def _is_pl_notifications() -> bool:
        lang_pref = hass.data[DOMAIN].get("tablet_language", "auto")
        if lang_pref == "auto":
            lang_pref = (getattr(getattr(hass, "config", None), "language", "en") or "en").lower()
        return str(lang_pref).startswith("pl")
    ...
    async def handle_read_safe_device_config(call: ServiceCall):
        ...
        for device_uuid in uuids:
            ...
            is_pl = _is_pl_notifications()
            notif_title = "Visionect Safe Config - Odczyt" if is_pl else "Visionect Safe Config - Read"
            has_readback = _has_any_read_value(read_data.get("values", {}))
            if is_pl:
                note = (
                    "Uwaga: odczyt jest zależny od firmware, czasem urządzenie zwraca puste wartości mimo poprawnego zapisu.\n\n"
                    if has_readback
                    else
                ...
                notif_body = (
                    "Krok 1/3 zakończony: konfiguracja została odczytana z tabletu.\n\n"
                    f"Urządzenie: `{device_uuid}`\n\n"
                    f"{_format_safe_values(read_data.get('values', {}))}\n\n"
                    f"{note}"
                    "Następny krok: uruchom `visionect_joan.apply_safe_device_config`, aby zapisać wybrane wartości."
                )
            else:
                note = (
                    "Note: read results depend on firmware; device may return empty values even when writes succeed.\n\n"
                    if has_readback
                    else
                ...
                notif_body = (
                    "Step 1/3 completed: configuration read from tablet.\n\n"
                    f"Device: `{device_uuid}`\n\n"
                    f"{_format_safe_values(read_data.get('values', {}))}\n\n"
                    f"{note}"
                    "Next step: run `visionect_joan.apply_safe_device_config` to save selected values."
                )
            async_create_persistent_notification(
                hass,
                notif_body,
        ...
        for device_uuid in uuids:
            ...
            if not values_to_write:
                ...
                async_create_persistent_notification(
                    ...
                    title="Visionect Safe Config - Brak mapowania" if is_pl else "Visionect Safe Config - Mapping Missing",
            ...
            changed_view = ", ".join(SAFE_FIELD_LABELS.get(k, k) for k in written_keys)
            is_pl = _is_pl_notifications()
            if success:
                verified = await _read_safe_config_for_uuid(device_uuid)
                state["last_read"][device_uuid] = verified
                verified_values = verified.get("values", {})
                requested_values = {k: _encode_safe_value(k, v) for k, v in incoming.items()}
                verify_report = _format_verify_report(requested_values, verified_values, is_pl)
                async_create_persistent_notification(
                    hass,
                    (
                        "Zapisano konfigurację urządzenia.\n\n"
                        f"Urządzenie: `{device_uuid}`\n"
                        f"Zmienione pola: {changed_view or '-'}\n\n"
                        f"Ustawione wartości:\n{_format_applied_values(requested_values, True)}"
                        if is_pl
                        else
                        "Device configuration saved.\n\n"
                        f"Device: `{device_uuid}`\n"
                        f"Changed fields: {changed_view or '-'}\n\n"
                        f"Applied values:\n{_format_applied_values(requested_values, False)}"
                    ),
                    title="Visionect Safe Config - Zapisano" if is_pl else "Visionect Safe Config - Applied",
                    notification_id=f"visionect_safe_apply_{device_uuid}",
                )
            else:
                async_create_persistent_notification(
                    ...
                    title="Visionect Safe Config - Błąd zapisu" if is_pl else "Visionect Safe Config - Apply Failed",
                    notification_id=f"visionect_safe_apply_{device_uuid}",
                )
            hass.bus.async_fire(
                EVENT_COMMAND_RESULT,
        ...
        for device_uuid in uuids:
            ...
            if not backup:
                async_create_persistent_notification(
                    ...
                    title="Visionect Safe Config - Brak backupu" if is_pl else "Visionect Safe Config - No Backup",
            ...
            if success:
                ...
                async_create_persistent_notification(
                    ...
                    title="Visionect Safe Config - Przywrócono" if is_pl else "Visionect Safe Config - Restored",
                    notification_id=f"visionect_safe_restore_{device_uuid}",
                )
            else:
                async_create_persistent_notification(
                    hass,
                    (
                        "Rollback nie powiódł się podczas zapisu poprzednich ustawień.\n\n"
                        f"Urządzenie: `{device_uuid}`"
                        if is_pl
                        else
                        "Rollback failed while writing previous safe settings.\n\n"
                        f"Device: `{device_uuid}`"
                    ),
                    title="Visionect Safe Config - Błąd przywracania" if is_pl else "Visionect Safe Config - Restore Failed",
                    notification_id=f"visionect_safe_restore_{device_uuid}",
                )
            hass.bus.async_fire(
                EVENT_COMMAND_RESULT,
