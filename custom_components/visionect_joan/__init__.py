...
from .notification_i18n import (
    ...
    vss_error_code_display,
    vss_error_code_description,
)
from .const import (
    ...
    CONF_ERROR_CODE_HA_NOTIFICATION_ENABLED,
    CONF_OFFLINE_HA_NOTIFICATION_ENABLED,
    CONF_OFFLINE_THRESHOLD_HOURS,
    CONF_HEALTH_HA_NOTIFICATION_ENABLED,
    OFFLINE_THRESHOLD_HOUR_CHOICES,
    ...
    vss_error_code_is_nonzero,
    vss_error_code_normalize,
...
from . import repairs as visionect_repairs
...
def _notify_device_error_code_if_needed(
    hass: HomeAssistant,
    guard_state: dict,
    uuid_val: str,
    device_name: str,
    ...
    *,
    ...
    """Persistent notification when VSS Status.ErrorCode is non-zero (once per code change)."""
    status = device_details.get("Status") or {}
    ec_raw = status.get("ErrorCode")
    notif_id = f"visionect_error_{uuid_val}"
    notified = guard_state.setdefault("error_code_notified", {})

    if not enabled:
        if uuid_val in notified:
            notified.pop(uuid_val, None)
            try:
                async_dismiss_persistent_notification(hass, notif_id)
            except Exception:
                pass
        return

    if not vss_error_code_is_nonzero(ec_raw):
        if uuid_val in notified:
            notified.pop(uuid_val, None)
            try:
                async_dismiss_persistent_notification(hass, notif_id)
    ...
    code_num = vss_error_code_normalize(ec_raw)
    if code_num is None:
        return
    if notified.get(uuid_val) == code_num:
        return

    notified[uuid_val] = code_num
    nlang = _tablet_notify_lang(hass)
    description = vss_error_code_description(nlang, code_num)
    connect_reason = str(status.get("ConnectReason", "")).strip()
    if connect_reason and connect_reason.lower() not in UNKNOWN_STRINGS:
        description = (
            f"{description}\n\n"
            f"{ntr(nlang, 'error_code_connect_reason', reason=connect_reason)}"
        )
    ...
    try:
        async_create_persistent_notification(
            ...
            ntr(
                ...
                code=vss_error_code_display(ec_raw),
                description=description,
            ),
            title=ntr(nlang, "error_code_title", device_name=device_name),
            notification_id=notif_id,
        )
        _LOGGER.info(
            "ErrorCode guard (HA notify) for %s: %s",
            device_name,
            vss_error_code_display(ec_raw),
        )
    except Exception as notify_err:
        _LOGGER.debug(
            "Could not create ErrorCode persistent notification for %s: %s",
            device_name,
            notify_err,
        )


def _offline_threshold_hours_from_options(options: dict) -> int:
    """Resolve offline notification threshold (1 / 4 / 24 h)."""
    try:
        hours = int(options.get(CONF_OFFLINE_THRESHOLD_HOURS, 4))
    except (TypeError, ValueError):
        hours = 4
    if hours not in OFFLINE_THRESHOLD_HOUR_CHOICES:
        hours = 4
    return hours


def _offline_duration_hours(offline_duration: timedelta) -> int:
    """Whole hours offline (floor), minimum 1 when duration is positive."""
    if offline_duration.total_seconds() <= 0:
        return 0
    return max(1, int(offline_duration.total_seconds() // 3600))


def _notify_offline_if_needed(
    hass: HomeAssistant,
    guard_state: dict,
    uuid_val: str,
    device_name: str,
    *,
    ...
    """Persistent notification when a tablet stays offline longer than threshold.

    Fires when threshold is crossed, then updates the same notification each time
    the offline duration increases by at least one whole hour (4 → 5 → 8 …).
    """
    notif_id = f"visionect_offline_{uuid_val}"

    # uuid -> last whole-hour count shown in the notification body
    notified_hours: dict[str, int] = guard_state.setdefault("offline_notified_hours", {})
    legacy = guard_state.pop("offline_notified", None)
    if isinstance(legacy, set):
        for legacy_uuid in legacy:
            notified_hours.setdefault(str(legacy_uuid), threshold_hours)

    if not enabled:
        if uuid_val in notified_hours:
            notified_hours.pop(uuid_val, None)
            try:
                async_dismiss_persistent_notification(hass, notif_id)
            except Exception:
                pass
    ...
    if is_online:
        ...
        try:
            ...
            pass
        mark = hass.data.get(DOMAIN, {}).get("guard_persist_mark")
        if callable(mark):
            mark()
        return

    last_seen = guard_state.get("last_seen", {}).get(uuid_val)
    if not last_seen:
        return

    offline_duration = datetime.now(timezone.utc) - last_seen
    if offline_duration <= timedelta(hours=threshold_hours):
        return

    offline_hours = _offline_duration_hours(offline_duration)
    last_shown_hours = notified_hours.get(uuid_val)
    if last_shown_hours is not None and offline_hours <= last_shown_hours:
        return

    notified_hours[uuid_val] = offline_hours
    nlang = _tablet_notify_lang(hass)
    try:
        async_create_persistent_notification(
            hass,
            ...
            title=ntr(nlang, "offline_title"),
            notification_id=notif_id,
        )
        _LOGGER.info(
            "Connection guard (HA notify) for %s (%sh offline, threshold %sh)",
            device_name,
            offline_hours,
            threshold_hours,
        )
...
def _notify_health_orphan_if_needed(
    hass: HomeAssistant,
    guard_state: dict,
    uuid_val: str,
    device_name: str,
    ...
    *,
    ...
    """Persistent notification when VSS reports an orphan / session problem."""
    notif_id = f"joan_orphan_{uuid_val}"
    notified = guard_state.setdefault("health_orphan_notified", {})
    orphan_error = device_details.get("OrphanError")
    error_text = str(orphan_error).strip() if orphan_error else ""
    ...
    if not error_text or error_text.lower() in UNKNOWN_STRINGS:
        if uuid_val in notified:
            notified.pop(uuid_val, None)
            try:
                async_dismiss_persistent_notification(hass, notif_id)
    ...
    if notified.get(uuid_val) == error_text:
        return

    notified[uuid_val] = error_text
    nlang = _tablet_notify_lang(hass)
    try:
        async_create_persistent_notification(
            hass,
            ntr(nlang, "health_check_problem", error=error_text),
            title=ntr(nlang, "health_check_title", device_name=device_name),
            notification_id=notif_id,
        )
        _LOGGER.info(
            "Health guard (HA notify) for %s: %s",
            device_name,
            error_text,
        )
    except Exception as notify_err:
        _LOGGER.debug(
            "Could not create health persistent notification for %s: %s",
            device_name,
            notify_err,
        )
...
async def _build_log_analysis_diagnostics_section(
    ...
    *,
    ...
    try:
        ...
        orphan_error = str(api.orphan_error_for_uuid(orphan_map, device_uuid) or "").strip()
    except Exception:
        orphan_error = ""
...
async def async_run_ollama_device_analysis_for_device(
    ...
    async def _run() -> None:
        ...
        try:
            ...
            orphan_err = api.orphan_error_for_uuid(orphans, device_uuid)
            if orphan_err:
                pr = {**pr, "OrphanError": orphan_err}
        except Exception:
            pass
        try:
...
INTERACTIVE_SCHEMA_EXTENSION = {
    ...
    vol.Optional(ATTR_WAKE_TABLET, default=False): cv.boolean,
}
...
SERVICE_RELOAD_URL_SCHEMA = vol.Schema({
    vol.Required(ATTR_DEVICE_ID): vol.Any(cv.string, [cv.string]),
    vol.Optional(ATTR_URL): cv.string,
    vol.Optional(ATTR_PREDEFINED_URL): cv.string,
})
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    guard_state = hass.data[DOMAIN].setdefault("guard_state", {
        ...
        "error_code_notified": {},
        "health_orphan_notified": {},
    })
    guard_state.setdefault("low_battery_svg_next_alt", {})
    guard_state.setdefault("error_code_notified", {})
    guard_state.setdefault("health_orphan_notified", {})

    from .guard_store import GuardPersistScheduler, async_load_guard_persist

    guard_persist = GuardPersistScheduler(hass, entry.entry_id)
    persisted_guard = await async_load_guard_persist(hass, entry.entry_id)
    guard_state["last_seen"].update(persisted_guard.get("last_seen") or {})
    guard_state["offline_notified_hours"].update(
        persisted_guard.get("offline_notified_hours") or {}
    )

    def _guard_persist_mark_dirty() -> None:
        guard_persist.mark_dirty()

    hass.data[DOMAIN]["guard_persist_mark"] = _guard_persist_mark_dirty
    ...
    async def async_update_data():
        try:
            ...
            for device_entry in devices_summary:
                ...
                device_details["OrphanError"] = api.orphan_error_for_uuid(orphans, uuid_val)

                visionect_repairs.async_sync_device_orphan_issue(
                    hass,
                    entry.entry_id,
                    uuid_val,
                    orphan_error=device_details.get("OrphanError"),
                    device_name=str(device_details.get("Options", {}).get("Name") or uuid_val),
                )

                # VSS State: online/offline (+ GUI: sleeping, charging — ładowanie ≠ offline)
                state_val = device_details.get("State", "").lower()
                is_online = is_vss_device_connected(state_val)
                
                now_utc = datetime.now(timezone.utc)
                if is_online:
                    prev_seen = guard_state["last_seen"].get(uuid_val)
                    guard_state["last_seen"][uuid_val] = now_utc
                    if prev_seen != now_utc:
                        _guard_persist_mark_dirty()
                ...
                _opts_notify = entry.options
                offline_ha_on = bool(
                    _opts_notify.get(CONF_OFFLINE_HA_NOTIFICATION_ENABLED, True)
                )
                offline_thr = _offline_threshold_hours_from_options(_opts_notify)
                _notify_offline_if_needed(
                    ...
                    device_name,
                    is_online=is_online,
                    enabled=offline_ha_on,
                    threshold_hours=offline_thr,
                )

                error_ha_on = bool(
                    _opts_notify.get(CONF_ERROR_CODE_HA_NOTIFICATION_ENABLED, True)
                )
                _notify_device_error_code_if_needed(
                    hass,
                    guard_state,
                    uuid_val,
                    device_name,
                ...
                health_ha_on = bool(
                    _opts_notify.get(CONF_HEALTH_HA_NOTIFICATION_ENABLED, False)
                )
                _notify_health_orphan_if_needed(
                    hass,
                    guard_state,
                    uuid_val,
                    device_name,
                    device_details,
                    enabled=health_ha_on,
                )
    ...
    async def _async_gather_recovery_uuids(
        ...
        if not uuids and coordinator.data:
            ...
            for u, d in coordinator.data.items():
                ...
                if st == "offline":
                    continue
                if is_vss_device_connected(st) or st == "":
                    uuids.append(u)
    ...
    async def _async_run_session_batch_recovery(
        ...
        *,
        ...
        from .recovery_eco import filter_recovery_uuids_eco

        entry_data_live = hass.data.get(DOMAIN, {}).get(entry.entry_id) or {}
        uuids, skipped_eco = filter_recovery_uuids_eco(
            uuids,
            coordinator,
            entry_data_live,
            api,
        ...
        if skipped_eco:
            _LOGGER.info(
                "%s: Eco recovery skip — session URL OK for %d tablet(s): %s",
                log_ctx,
                len(skipped_eco),
                ", ".join(skipped_eco[:5]),
            )
        ...
        while True:
            try:
                ...
                if is_healthy and vss_was_offline:
                    ...
                    if not wd_logged_offline:
                        _LOGGER.warning("VSS Watchdog: VSS went offline. Waiting for recovery...")
                        wd_logged_offline = True
                    visionect_repairs.async_sync_vss_offline_issue(hass, entry.entry_id, True)
                    vss_was_offline = True
                    recovery_attempts_no_uuids = 0
                    wd_state["quick_warmup"] = False
                    healthy_check_count = 0
                ...
                if is_healthy and not vss_was_offline:
                    if wd_logged_offline:
                        wd_logged_offline = False
                    visionect_repairs.async_sync_vss_offline_issue(hass, entry.entry_id, False)
    ...
    async def _build_per_device_coordinators() -> None:
        ...
        async def _create_device_coordinator(target_uuid: str) -> DataUpdateCoordinator:
            ...
            async def _update_one() -> dict:
                ...
                orphans = await api.async_get_orphans()
                details["OrphanError"] = api.orphan_error_for_uuid(orphans, target_uuid)
                details["LastUpdated"] = datetime.now(timezone.utc)

                state_val = str(details.get("State", "")).lower()
                is_online = is_vss_device_connected(state_val)
                now_utc = datetime.now(timezone.utc)
                if is_online:
                    prev_seen = guard_state["last_seen"].get(target_uuid)
                    guard_state["last_seen"][target_uuid] = now_utc
                    if prev_seen != now_utc:
                        _guard_persist_mark_dirty()

                _opts_dev = entry.options
                offline_ha_on = bool(
                    _opts_dev.get(CONF_OFFLINE_HA_NOTIFICATION_ENABLED, True)
                )
                offline_thr = _offline_threshold_hours_from_options(_opts_dev)
                _notify_offline_if_needed(
                    hass,
                    guard_state,
                    target_uuid,
                ...
                error_ha_on = bool(
                    _opts_dev.get(CONF_ERROR_CODE_HA_NOTIFICATION_ENABLED, True)
                )
                _notify_device_error_code_if_needed(
                    hass,
                    guard_state,
                ...
                health_ha_on = bool(
                    _opts_dev.get(CONF_HEALTH_HA_NOTIFICATION_ENABLED, False)
                )
                _notify_health_orphan_if_needed(
                    hass,
                    guard_state,
        ...
        async def _async_ollama_tick(_now: datetime) -> None:
            ...
            async with ollama_job_lock:
                try:
                    ...
                    try:
                        devices = await api.async_get_all_devices() or []
                        orphans = await api.async_get_orphans(include_deferred=True) or {}
    ...
    def _unwrap_back_target_url(url: str | None) -> str | None:
        """Normalize a stored/navigation back target (unwrap recovery/apply, drop data: URLs)."""
        if not url:
            return None
        u = str(url).strip()
        ...
        if "/api/visionect_joan/recovery/apply" in u:
            try:
                ...
                if target:
                    inner = urllib.parse.unquote(str(target).strip())
                    if inner and not inner.startswith("data:"):
                        return inner
            except Exception:
                return None
            return None
    ...
    def _resolve_back_target(device_uuid: str, call_data: dict) -> str | None:
        """Resolved http(s) back target from service call, prefs, or main menu."""
        ...
        if back_url:
            if not back_url.startswith(("http", "data:")):
                ...
                unwrapped = _unwrap_back_target_url(resolved)
            else:
                unwrapped = _unwrap_back_target_url(back_url)
            if unwrapped:
                return _expand_uuid_placeholder(unwrapped, device_uuid)
            return None

        prefs_local = _get_prefs()
        for pref_key in ("back_targets", "view_targets_by_uuid"):
            stored = _unwrap_back_target_url(prefs_local.get(pref_key, {}).get(nu))
            if stored:
                return _expand_uuid_placeholder(stored, device_uuid)

        unwrapped_menu = _unwrap_back_target_url(main_menu)
        if unwrapped_menu:
            return _expand_uuid_placeholder(unwrapped_menu, device_uuid)
        return None
    ...
    async def _async_capture_back_target_before_overlay(device_uuid: str) -> None:
        ...
        url = _unwrap_back_target_url(url) or ""
    ...
    def _get_back_url_for_uuid(device_uuid: str, call_data: dict) -> str | None:
        resolved = _resolve_back_target(device_uuid, call_data)
    ...
    async def handle_diagnose_vss_session(call: ServiceCall):
        ...
        if check_device_online:
            ...
            orphans_resp = await api.async_get_orphans(include_deferred=True)
            orphan_by_uuid = orphans_resp if isinstance(orphans_resp, dict) else {}
    ...
    async def _service_push_and_fire(
        device_uuid: str,
        url: str,
        call: ServiceCall,
        service_name: str,
        *,
        ...
        """Battery-aware session URL push + command_result event."""
        entry_data = hass.data[DOMAIN][entry.entry_id]
        result, skipped = await async_push_url_from_service(
            ...
            url,
            call.data,
            intentional_wake=intentional_wake,
        )
        fire_command_result(
            hass, device_uuid, service_name, result, skipped_wake=skipped
        )
        return result
    ...
    async def handle_set_url(call: ServiceCall):
        ...
        for uuid_val in uuids:
            ...
            if final_url.startswith("data:"):
                ...
                fire_command_result(hass, uuid_val, SERVICE_SET_URL, "failure")
            else:
                final_url = create_simple_cache_buster(final_url)
                await _service_push_and_fire(uuid_val, final_url, call, SERVICE_SET_URL)
        await _async_refresh_device_coordinators(uuids)

    async def handle_reload_url(call: ServiceCall):
        """Battery-expensive: clear WebKit cache → forced set_url → session restart.

        Use only when the same page path has new content and wake_tablet alone is not enough.
        """
        uuids = await get_uuids_from_call(call)
        original_url = _get_url_from_params(call.data, ATTR_URL, ATTR_PREDEFINED_URL)
        if not original_url:
            _LOGGER.error("No URL provided for reload_url.")
            return

        _LOGGER.warning(
            "reload_url on %s device(s): full cache clear + session restart (e-ink battery cost).",
            len(uuids),
        )
        await api.async_clear_webkit_cache(uuids)
        ...
        for uuid_val in uuids:
            ...
            if final_url.startswith("data:"):
                _LOGGER.error(
                    "reload_url: could not publish URL for %s (data URI or cache write failed).",
                    uuid_val,
                )
                fire_command_result(hass, uuid_val, SERVICE_RELOAD_URL, "failure")
                continue
            final_url = create_simple_cache_buster(final_url)
            result = await _service_push_and_fire(
                uuid_val,
                final_url,
                call,
                SERVICE_RELOAD_URL,
                intentional_wake=True,
            )
            if result == "success":
                restarted = await api.async_restart_sessions_batch([uuid_val])
                if not restarted:
                    _LOGGER.warning(
                        "reload_url: URL set for %s but session restart failed.",
                        uuid_val,
                    )
        await _async_refresh_device_coordinators(uuids)
    ...
    async def handle_send_text(call: ServiceCall):
        ...
        for device_uuid in uuids:
            ...
            await _service_push_and_fire(device_uuid, final_url, call, SERVICE_SEND_TEXT)
    ...
    async def handle_send_weather(call: ServiceCall):
        ...
        for device_uuid in uuids:
            ...
            if cached_url:
                await _service_push_and_fire(device_uuid, cached_url, call, SERVICE_SEND_WEATHER)
                continue
            weather_state = hass.states.get(weather_entity_id)
            if not weather_state:
                stale_url = _screen_cache_get_stale(entry_data_cache, cache_key)
                if stale_url:
                    await _service_push_and_fire(device_uuid, stale_url, call, SERVICE_SEND_WEATHER)
                continue
            ...
            await _service_push_and_fire(device_uuid, final_url, call, SERVICE_SEND_WEATHER)
    ...
    async def handle_send_energy_panel(call: ServiceCall):
        ...
        for device_uuid in uuids:
            ...
            cached_url = _screen_cache_get(entry_data_cache, cache_key, cache_ttl_s)
            if cached_url:
                await _service_push_and_fire(device_uuid, cached_url, call, SERVICE_SEND_ENERGY_PANEL)
                continue
            ...
            final_url = await _process_final_url(hass, interactive_url)
            _screen_cache_put(entry_data_cache, cache_key, final_url)
            await _service_push_and_fire(device_uuid, final_url, call, SERVICE_SEND_ENERGY_PANEL)
    ...
    async def handle_send_todo_list(call: ServiceCall):
        ...
        for device_uuid in uuids:
            ...
            cached_url = _screen_cache_get(entry_data_cache, cache_key, cache_ttl_s)
            if cached_url:
                await _service_push_and_fire(device_uuid, cached_url, call, SERVICE_SEND_TODO_LIST)
                continue
            ...
            final_url = await _process_final_url(hass, interactive_url)
            _screen_cache_put(entry_data_cache, cache_key, final_url)
            await _service_push_and_fire(device_uuid, final_url, call, SERVICE_SEND_TODO_LIST)
    ...
    async def handle_send_rss_feed(call: ServiceCall):
        ...
        for device_uuid in uuids:
            ...
            cached_url = _screen_cache_get(entry_data_cache, cache_key, cache_ttl_s)
            if cached_url:
                await _service_push_and_fire(device_uuid, cached_url, call, SERVICE_SEND_RSS_FEED)
                continue
            back_url = _get_back_url_for_uuid(device_uuid, call.data)
            add_back = _effective_add_back_button(call, back_url)
            interactive_url = await _add_interactive_layer_to_url(
            ...
            final_url = await _process_final_url(hass, interactive_url)
            _screen_cache_put(entry_data_cache, cache_key, final_url)
            await _service_push_and_fire(device_uuid, final_url, call, SERVICE_SEND_RSS_FEED)
    ...
    async def handle_send_status_panel(call: ServiceCall):
        ...
        for device_uuid in uuids:
            ...
            cached_url = _screen_cache_get(entry_data_cache, cache_key, cache_ttl_s)
            if cached_url:
                await _service_push_and_fire(device_uuid, cached_url, call, SERVICE_SEND_STATUS_PANEL)
                continue
            ...
            final_url = await _process_final_url(hass, interactive_url)
            _screen_cache_put(entry_data_cache, cache_key, final_url)
            await _service_push_and_fire(device_uuid, final_url, call, SERVICE_SEND_STATUS_PANEL)
    ...
    async def handle_send_qr_code(call: ServiceCall):
        ...
        for device_uuid in uuids:
            ...
            final_url = await _process_final_url(hass, interactive_url)
            await _service_push_and_fire(device_uuid, final_url, call, SERVICE_SEND_QR_CODE)
    ...
    async def handle_send_calendar(call: ServiceCall):
        ...
        for device_uuid in uuids:
            ...
            final_url = await _process_final_url(hass, interactive_url)
            await _service_push_and_fire(device_uuid, final_url, call, SERVICE_SEND_CALENDAR)
    ...
    async def handle_send_camera_snapshot(call: ServiceCall):
        ...
        for device_uuid in uuids:
            ...
            final_url = await _process_final_url(hass, interactive_url)
            await _service_push_and_fire(device_uuid, final_url, call, SERVICE_SEND_CAMERA_SNAPSHOT)
    ...
    async def handle_send_sensor_graph(call: ServiceCall):
        ...
        for device_uuid in uuids:
            ...
            cached_url = _screen_cache_get(entry_data_cache, cache_key, cache_ttl_s)
            if cached_url:
                await _service_push_and_fire(device_uuid, cached_url, call, SERVICE_SEND_SENSOR_GRAPH)
                continue
            back_url = _get_back_url_for_uuid(device_uuid, call.data)
            try:
            ...
            final_url = await _process_final_url(hass, interactive_url)
            _screen_cache_put(entry_data_cache, cache_key, final_url)
            await _service_push_and_fire(device_uuid, final_url, call, SERVICE_SEND_SENSOR_GRAPH)
    ...
    async def handle_send_image_url(call: ServiceCall):
        ...
        for device_uuid in uuids:
            ...
            final_url = await _process_final_url(hass, interactive_url)
            await _service_push_and_fire(device_uuid, final_url, call, SERVICE_SEND_IMAGE_URL)
    ...
    async def handle_start_slideshow(call: ServiceCall):
        ...
        for device_uuid in uuids:
            ...
            final_url = await _process_final_url(hass, interactive_url)
            await _service_push_and_fire(device_uuid, final_url, call, SERVICE_START_SLIDESHOW)
    ...
    async def handle_send_keypad(call: ServiceCall):
        ...
        for device_uuid in uuids:
            ...
            final_url = await _process_final_url(hass, interactive_url)
            await _service_push_and_fire(device_uuid, final_url, call, SERVICE_SEND_KEYPAD)
    ...
    async def handle_send_crypto(call: ServiceCall):
        ...
        for device_uuid in uuids:
            ...
            final_url = await _process_final_url(hass, interactive_url)
            await _service_push_and_fire(device_uuid, final_url, call, SERVICE_SEND_CRYPTO)
    ...
    async def handle_send_button_panel(call: ServiceCall):
        ...
        for device_uuid in uuids:
            ...
            final_url = await _process_final_url(hass, interactive_url)
            await _service_push_and_fire(device_uuid, final_url, call, SERVICE_SEND_BUTTON_PANEL)
    ...
    hass.services.async_register(DOMAIN, SERVICE_RELOAD_URL, handle_reload_url, schema=SERVICE_RELOAD_URL_SCHEMA)
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    services_to_remove = [
        SERVICE_SET_URL, SERVICE_RELOAD_URL, SERVICE_SEND_TEXT, SERVICE_SET_DISPLAY_ROTATION, SERVICE_FORCE_REFRESH,
    ...
    if unload_ok := await hass.config_entries.async_unload_platforms(entry, PLATFORMS):
        entry_data = hass.data.get(DOMAIN, {}).get(entry.entry_id) or {}
        guard_persist = entry_data.get("guard_persist")
        if guard_persist is not None:
            try:
                await guard_persist.async_flush_now()
            except Exception:
                pass
