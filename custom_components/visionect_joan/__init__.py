...
def _notify_offline_if_needed(
    ...
    *,
    ...
    notified = guard_state.setdefault("offline_notified", set())

    if not enabled:
        if uuid_val in notified:
            notified.discard(uuid_val)
            try:
                async_dismiss_persistent_notification(hass, notif_id)
            except Exception:
                pass
    ...
    try:
        ...
        _LOGGER.info(
            "Connection guard (HA notify) for %s (>%sh offline)",
            device_name,
            threshold_hours,
        )
    except Exception as notify_err:
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    async def _build_per_device_coordinators() -> None:
        ...
        async def _create_device_coordinator(target_uuid: str) -> DataUpdateCoordinator:
            ...
            async def _update_one() -> dict:
                ...
                elif target_uuid not in guard_state["last_seen"]:
                    guard_state["last_seen"][target_uuid] = now_utc
    ...
    async def handle_send_text(call: ServiceCall):
        ...
        for device_uuid in uuids:
            ...
            status = "success" if await api.async_set_device_url(device_uuid, final_url, force=True) else "failure"
    ...
    async def handle_send_weather(call: ServiceCall):
        ...
        for device_uuid in uuids:
            ...
            if cached_url:
                status = "success" if await api.async_set_device_url(device_uuid, cached_url, force=True) else "failure"
            ...
            if not weather_state:
                ...
                if stale_url:
                    status = "success" if await api.async_set_device_url(device_uuid, stale_url, force=True) else "failure"
                    hass.bus.async_fire(EVENT_COMMAND_RESULT, {"uuid": device_uuid, "service": SERVICE_SEND_WEATHER, "status": status})
                continue
    ...
    async def handle_send_camera_snapshot(call: ServiceCall):
        ...
        for device_uuid in uuids:
            ...
            status = (
                "success"
                if await api.async_set_device_url(device_uuid, final_url, force=True)
                else "failure"
            )
