...
from homeassistant.core import HomeAssistant, ServiceCall, State, ServiceResponse
...
from homeassistant.helpers.storage import Store # NEW IMPORT
...
from .const import (
    ...
    NETWORK_RETRY_DELAY, ATTR_PREDEFINED_URL, ATTR_PREDEFINED_BACK_URL
)
...
INTERACTIVE_SCHEMA_EXTENSION = {
    ...
    vol.Optional(ATTR_PREDEFINED_BACK_URL): cv.string,
...
def _add_interactive_layer_to_url(
    ...
    click_anywhere: bool,
    click_anywhere_to_action: bool,
    action_webhook_id: str | None = None,
) -> str:
    """Decode a data:text/html URL, inject buttons/actions, and encode it back."""
    if not (add_back_button or click_anywhere or click_anywhere_to_action or action_webhook_id):
        return data_url
    ...
    try:
        ...
        buttons_visible = not (click_anywhere_to_action or click_anywhere)
        
        padding_css = "body { margin: 0; padding-top: 100px !important; }" if buttons_visible else "body { margin: 0; }"
        ...
        *, *:before, *:after {{ box-sizing: inherit; }}
        ...
        .visionect-interactive-button {{
            position: absolute; top: 20px; width: 30%; display: flex; align-items: center; justify-content: center;
            padding: 12px 10px; background-color: #f2f2f2; color: #000; border: 3px solid #000;
            border-radius: 2px; text-decoration: none; font-size: 22px; font-weight: 700;
            line-height: 1; z-index: 1000; cursor: pointer; white-space: nowrap;
        }}
        ...
        if action_webhook_id:
            try:
                webhook_url = get_url(hass, internal=True, relative=False) + f"/api/webhook/{action_webhook_id}"
            except Exception as e:
                _LOGGER.error(f"Could not determine internal URL for webhook, falling back. Error: {e}")
                webhook_url = f"http://homeassistant.local:8123/api/webhook/{action_webhook_id}"
        ...
        if buttons_visible and add_back_button and back_url:
        ...
        if click_anywhere_to_action and webhook_url:
            interactive_scripts += f"""
            (function() {{
                document.addEventListener('DOMContentLoaded', function() {{ 
                    document.body.addEventListener('click', function(event) {{
                        triggerWebhook(null, '{webhook_url}');
                    }}, {{ capture: false }}); 
                }});
            }})();
            """
...
async def create_rss_feed_url(hass: HomeAssistant, title: str, items: list, lang: str, small_screen: bool) -> str:
    """Creates a data URL for an RSS feed display."""
    header_fs = "2.8em" if small_screen else "3.5em"
    item_fs = "1.8em" if small_screen else "2.2em"
    
    style_css = f"""
        body {{ font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif; background-color: white; color: black; margin: 0; padding: 30px; box-sizing: border-box; }}
        .header {{ text-align: center; font-size: {header_fs}; font-weight: bold; border-bottom: 3px solid black; padding-bottom: 20px; margin-bottom: 25px; }}
        ul {{ list-style: none; padding: 0; margin: 0; }}
        li {{ font-size: {item_fs}; padding: 15px 5px; border-bottom: 2px solid #eee; line-height: 1.3; }}
        li:last-child {{ border-bottom: none; }}
        .icon {{ display: inline-block; margin-right: 15px; }}
        .error {{ text-align: center; font-size: 2.0em; padding-top: 50px; color: #888; }}
    """
    ...
    if not items:
        items_html = f"<div class='error'>{error_text}</div>"
    else:
        for item in items:
            summary = html.escape(item.get('title', 'Brak tytułu'))
            items_html += f"""
            <li>
                <img src="{icon_bullet}" class="icon" style="width: 0.8em; height: 0.8em;" />
                {summary}
            </li>
            """
...
async def create_energy_panel_url(hass: HomeAssistant, states: dict[str, State | None], orientation: str, lang: str, small_screen: bool) -> str:
    ...
    # POPRAWKA: Według dokumentacji Visionect, 0 i 2 to Portrait (pionowo)
...
# ---- Graph generation helper for sensors ----
def _generate_graph_image(
    ...
    try:
        import numpy as np  # optional helper if present
    except Exception:
        # numpy optional; proceed without it
        pass
...
# ----------------- MODIFIED/NEW Helpers for predefined views -----------------
def _get_url_from_predefined(hass: HomeAssistant, predefined_name: str | None) -> str | None:
    """Get URL from predefined name in configuration.yaml."""
    if not predefined_name:
        return None
    views = hass.data[DOMAIN].get("views", [])
    for view in views:
        if view.get("name") == predefined_name:
            return view.get("url")
    return None

def _get_url_from_params(hass: HomeAssistant, call_data: dict, url_key: str, predefined_key: str) -> str | None:
    """Get URL from direct param or predefined in config."""
    direct_url = call_data.get(url_key)
    if direct_url:
        return direct_url
    predefined_name = call_data.get(predefined_key)
    return _get_url_from_predefined(hass, predefined_name)

def _get_prefs(hass: HomeAssistant, entry_id: str) -> dict:
    """Get preferences dictionary for the config entry."""
    return hass.data[DOMAIN][entry_id].setdefault("prefs", {"back_targets": {}})

def _get_back_url_for_uuid(hass: HomeAssistant, entry_id: str, device_uuid: str, call_data: dict) -> str | None:
    """
    Get the back button URL for a specific device based on priority:
    1. Explicit URL from service call (back_button_url, predefined_back_url).
    2. Stored user preference for the device (from VisionectBackViewSelect).
    3. Global fallback (main_menu_url).
    """
    # 1. Check for explicit URL in the service call
    back_url = call_data.get(ATTR_BACK_BUTTON_URL)
    if back_url:
        # If it's not a full URL, treat it as a predefined view name
        if not (back_url.startswith("http") or back_url.startswith("data:text/html,")):
            resolved = _get_url_from_predefined(hass, back_url)
            if resolved:
                return resolved
        return back_url

    predefined_back = call_data.get(ATTR_PREDEFINED_BACK_URL)
    if predefined_back:
        resolved = _get_url_from_predefined(hass, predefined_back)
        if resolved:
            return resolved

    # 2. Check for stored preference for this device
    prefs = _get_prefs(hass, entry_id)
    stored_url = prefs.get("back_targets", {}).get(device_uuid)
    if stored_url:
        return stored_url

    # 3. Fallback to the global main_menu_url
    return hass.data[DOMAIN].get("main_menu_url")


# ----------------- Setup entry and services -----------------
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    # NEW: Initialize storage for back button preferences
    store = Store(hass, 1, f"{DOMAIN}_{entry.entry_id}_prefs.json")
    prefs = await store.async_load() or {"back_targets": {}}
    hass.data[DOMAIN][entry.entry_id]["prefs_store"] = store
    hass.data[DOMAIN][entry.entry_id]["prefs"] = prefs
    ...
    async def handle_set_url(call: ServiceCall):
        ...
        # MODIFIED: Get URL from 'url' or 'predefined_url'
        original_url = _get_url_from_params(hass, call.data, ATTR_URL, ATTR_PREDEFINED_URL)

        # MODIFIED: Fallback for resolving a view name passed in the 'url' field
        if original_url and not (
            original_url.startswith("http://")
            or original_url.startswith("https://")
            or original_url.startswith("data:text/html,")
        ):
            resolved = _get_url_from_predefined(hass, original_url)
            if resolved:
                _LOGGER.info(f"Resolved predefined view name '{original_url}' to URL '{resolved}'")
                original_url = resolved
            else:
                _LOGGER.error(
                    f"Provided value '{original_url}' is not a valid URL and no predefined view with that name was found."
                )
                return

        if not original_url:
            _LOGGER.error("No URL provided. Please specify either 'url' or 'predefined_url'")
            return
        ...
        for i, uuid_val in enumerate(uuids):
            ...
            hass.bus.async_fire(
                ...
                {"uuid": uuid_val, "service": SERVICE_SET_URL, "status": status, "params": {"url": original_url}},
            )
    ...
    async def handle_send_text(call: ServiceCall):
        ...
        # MODIFIED: Per-device back URL and API call
        for device_uuid in uuids:
            back_url = _get_back_url_for_uuid(hass, entry.entry_id, device_uuid, call.data)
            final_url = _add_interactive_layer_to_url(
                ...
                click_anywhere_to_action=call.data[ATTR_CLICK_ANYWHERE_TO_ACTION],
            ...
            hass.bus.async_fire(EVENT_COMMAND_RESULT, {"uuid": device_uuid, "service": SERVICE_SEND_TEXT, "status": status, "params": {"message": message}})
    ...
    async def handle_send_weather(call: ServiceCall):
        ...
        for device_uuid in uuids:
            ...
            _LOGGER.info(f"Sending weather '{weather_entity_id}' to {device_uuid}.")
            status = "failure"
            try:
    ...
    async def handle_send_energy_panel(call: ServiceCall):
        ...
        for device_uuid in uuids:
            ...
            _LOGGER.info(f"Sending energy panel to {device_uuid}.")
            status = "failure"
            try:
                if await api.async_set_device_url(device_uuid, final_url): status = "success"
    ...
    async def handle_send_todo_list(call: ServiceCall):
        ...
        for device_uuid in uuids:
            ...
            _LOGGER.info(f"Sending to-do list '{todo_entity_id}' to {device_uuid}.")
            status = "failure"
            try:
                if await api.async_set_device_url(device_uuid, final_url): status = "success"
    ...
    async def handle_send_rss_feed(call: ServiceCall):
        uuids = await get_uuids_from_call(call)
        feed_url = call.data["feed_url"]
        title = call.data["title"]
        max_items = call.data["max_items"]
        lang = _get_lang(hass)
        small_screen = call.data[ATTR_SMALL_SCREEN]
        ...
        def _parse_feed():
            try:
                ...
                if feed.bozo:
                    _LOGGER.warning(f"Malformed RSS feed at {feed_url}: {feed.bozo_exception}")
                return [{"title": entry.title} for entry in feed.entries[:max_items]]
            except Exception as e:
                _LOGGER.error(f"Error fetching or parsing RSS feed at {feed_url}: {e}")
                return []
        ...
        for device_uuid in uuids:
            ...
            _LOGGER.info(f"Sending RSS feed from '{feed_url}' to {device_uuid}.")
            status = "failure"
            try:
                if await api.async_set_device_url(device_uuid, final_url):
                    status = "success"
    ...
    async def handle_send_status_panel(call: ServiceCall):
        ...
        for device_uuid in uuids:
            ...
            _LOGGER.info(f"Sending status panel to {device_uuid}.")
            status = "failure"
            try:
                if await api.async_set_device_url(device_uuid, final_url): status = "success"
    ...
    async def handle_send_qr_code(call: ServiceCall):
        ...
        for device_uuid in uuids:
            ...
            _LOGGER.info(f"Sending QR code to {device_uuid}.")
            status = "failure"
            try:
                if await api.async_set_device_url(device_uuid, final_url): status = "success"; _LOGGER.info(f"QR code successfully sent to {device_uuid}")
    ...
    async def handle_send_calendar(call: ServiceCall):
        ...
        for device_uuid in uuids:
            ...
            _LOGGER.info(f"Sending calendar '{calendar_entity_id}' to {device_uuid}.")
            status = "failure"
            try:
            ...
            hass.bus.async_fire(EVENT_COMMAND_RESULT, {"uuid": device_uuid, "service": SERVICE_SEND_CALENDAR, "status": status, "params": {"calendar": calendar_entity_id}})
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    hass.services.async_remove(DOMAIN, SERVICE_SEND_RSS_FEED)
