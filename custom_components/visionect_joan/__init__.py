"""
Visionect Joan Integration for Home Assistant.

This integration allows controlling Visionect e-ink displays via Home Assistant services.
It supports sending text, images, weather, calendar, RSS feeds, and more to devices.
"""
...
def _add_interactive_layer_to_url(
    hass: HomeAssistant, data_url: str, back_url: str | None, add_back_button: bool,
    click_anywhere: bool, click_anywhere_to_action: bool, action_webhook_id: str | None
) -> str:
    ...
    try:
        ...
        common_css = f"""
        html {{ box-sizing: border-box; }} *, *:before, *:after {{ box-sizing: inherit; }} {padding_css}
        .visionect-interactive-button {{ position: absolute; top: 20px; width: 30%; display: flex; align-items: center; justify-content: center; padding: 12px 10px; background-color: #f2f2f2; color: #000; border: 3px solid #000; border-radius: 2px; text-decoration: none; font-size: 22px; font-weight: 700; line-height: 1; z-index: 1000; cursor: pointer; white-space: nowrap; }}
        .visionect-interactive-button:active {{ opacity: 0.6; }} .visionect-back-button {{ left: 15%; }} .visionect-action-button {{ right: 15%; }}
        """
        ...
        interactive_scripts = ""
        if webhook_url:
            interactive_scripts += f"""function triggerWebhook(event, url) {{ if (event) {{ event.stopPropagation(); }} try {{ fetch(url, {{ method: 'POST', body: '{{}}' }}).catch(err => console.error('Webhook failed:', err)); }} catch(e) {{ console.error('Error in fetch:', e); }} }}"""
        if click_anywhere_to_action and webhook_url:
            interactive_scripts += f"""(function() {{ document.addEventListener('DOMContentLoaded', function() {{ document.body.addEventListener('click', function(event) {{ triggerWebhook(null, '{webhook_url}'); }}, {{ capture: false }}); }}); }})();"""
        elif click_anywhere and back_url:
            interactive_scripts += f"""(function() {{ function goBack() {{ try {{ window.location.href = "{back_url}"; }} catch(e) {{}} }} document.addEventListener('DOMContentLoaded', function() {{ document.body.addEventListener('click', goBack, {{ capture: false }}); }}); }})();"""
...
async def async_setup(hass: HomeAssistant, config: ConfigType) -> bool:
    """Set up the Visionect integration from YAML configuration."""
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    def _get_url_from_predefined(predefined_name: str | None) -> str | None:
        ...
        for view in hass.data[DOMAIN].get("views", []):
    ...
    def _get_url_from_params(call_data: dict, url_key: str, predefined_key: str) -> str | None:
        ...
        if direct_url:
            if not (direct_url.startswith("http") or direct_url.startswith("data:text/html,")):
    ...
    async def handle_set_url(call: ServiceCall):
        ...
        if not original_url:
            _LOGGER.error("No URL provided. Please specify 'url' or 'predefined_url'")
            return
    ...
    async def handle_send_text(call: ServiceCall):
        ...
        content_url = create_text_message_url(message, call.data[ATTR_TEXT_COLOR], call.data[ATTR_BACKGROUND_COLOR], f"{call.data[ATTR_TEXT_SIZE]}px", call.data[ATTR_TEXT_ALIGN], call.data[ATTR_FONT_FAMILY], call.data[ATTR_FONT_WEIGHT], call.data[ATTR_LAYOUT], call.data.get(ATTR_IMAGE_URL), call.data.get(ATTR_IMAGE_ZOOM))
        
        for device_uuid in uuids:
            back_url = _get_back_url_for_uuid(device_uuid, call.data)
            final_url = _add_interactive_layer_to_url(hass, content_url, back_url, call.data[ATTR_ADD_BACK_BUTTON], call.data[ATTR_CLICK_ANYWHERE_TO_RETURN], call.data[ATTR_CLICK_ANYWHERE_TO_ACTION], call.data.get(ATTR_ACTION_WEBHOOK_ID))
            status = "failure"
            try:
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    # Usunięcie rejestracji wszystkich usług
