...
async def async_run_ollama_device_analysis_for_device(
    ...
    async def _run() -> None:
        live: dict = {}
        try:
            fetched = await api.async_get_device_data(device_uuid)
        ...
        if not live:
            ...
            live = dict(cd) if isinstance(cd, dict) else {}
        ...
        if not ollama_ready:
            if context_excerpt:
                ...
                async_create_persistent_notification(
                    hass,
                    ntr(ai_lang, "log_analysis_no_data"),
                    title=title,
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    async def async_update_data():
        try:
            ...
            for device_entry in devices_summary:
                ...
                # Single fetch per device (list entries may omit SessionOptions; avoid double round-trip).
    ...
    async def _async_vss_watchdog():
        ...
        while True:
            try:
                ...
                poll_interval_s = 15.0 if (not is_healthy or vss_was_offline) else 60.0
    ...
    async def handle_set_url(call: ServiceCall):
        ...
        for uuid_val in uuids:
            ...
            status = "success" if await api.async_set_device_url(uuid_val, final_url) else "failure"
        ...
        for device_uuid in uuids:
            ...
            if not image_bytes:
                if not _check_matplotlib():
                    _LOGGER.warning(
                        "send_sensor_graph: matplotlib is not installed. "
                        "Sensor graphs need matplotlib (optional dependency — not listed in manifest so HAOS/ARM can load the integration). "
                        "Weather layout graphs behave the same."
                    )
                    content_url = f"data:text/html,{urllib.parse.quote('<html><body style=\"display:flex;align-items:center;justify-content:center;height:100vh;font-size:2em;padding:1em;text-align:center;\">Graphs need matplotlib<br/><small>(optional — see Visionect Joan docs)</small></body></html>')}"
                else:
