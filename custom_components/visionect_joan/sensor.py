...
# ---> NOWOŚĆ: Definiujemy maksymalną długość wyświetlanego URL <---
MAX_URL_DISPLAY_LENGTH = 55

# Słownik definicji sensorów
SENSOR_TYPES = {
...
class VisionectSensor(VisionectEntity, SensorEntity):
        ...
        try:
            ...
            if self.sensor_type == "configured_url":
                ...
                if url.startswith("data:text/html,"):
                    # <-- ZMIANA: Zamiast "Wiadomość tekstowa", sprawdzamy, czy to obrazek z kamery.
                    if "visionect_snapshot" in url:
                        return "Zdjęcie z kamery"
                    return "Treść dynamiczna (HTML)"
                
                if len(url) > MAX_URL_DISPLAY_LENGTH:
                    return f"{url[:MAX_URL_DISPLAY_LENGTH-3]}..."
    ...
    def extra_state_attributes(self):
        ...
        if self.sensor_type == "configured_url":
            ...
            # <-- ZMIANA: Dodajemy warunek, aby nie zapisywać gigantycznych URL-i 'data:' -->
