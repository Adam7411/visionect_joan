    ...
    def native_value(self):
        ...
        try:
            ...
            if self.sensor_type == "configured_url":
                ...
                # Pobierz aktualny adres wewnętrzny HA
                try:
                    current_ip = get_url(self.hass, allow_external=False, allow_ip=True)
                    # Usuń protokół dla czystszego porównania
                    current_ip_clean = current_ip.replace("http://", "").replace("https://", "").rstrip("/")
                except:
                    current_ip_clean = "YOUR_HA_IP"

                # Podmień dynamicznie
                if current_ip_clean in url:
                    display_url = url.replace(current_ip_clean, "HA_LOCAL")
                else:
                    display_url = url
