...
from .const import vss_error_code_normalize
from .html_i18n import normalize_lang

# Joan Wi‑Fi / VSS error codes → i18n key (decimal after normalize)
_VSS_ERROR_DESC_KEYS: dict[int, str] = {
    2: "error_code_desc_2",
    3: "error_code_desc_3",
    4: "error_code_desc_4",
    5: "error_code_desc_5",
    ...
    8: "error_code_desc_8",
    9: "error_code_desc_9",
    10: "error_code_desc_10",
    14: "error_code_desc_14",
    15: "error_code_desc_15",
    25: "error_code_desc_25",
    26: "error_code_desc_26",
    32: "error_code_desc_32",
    37: "error_code_desc_37",
    38: "error_code_desc_38",
    40: "error_code_desc_40",
    41: "error_code_desc_41",
    65: "error_code_desc_41",  # 0x0041 — same as 41 (PSK length)
    68: "error_code_desc_68",
    73: "error_code_desc_73",
    81: "error_code_desc_81",
    0xBB38: "error_code_desc_bb38",
    0xFF87: "error_code_desc_ff87",
    0xFF9C: "error_code_desc_ff9c",
    0xFF9D: "error_code_desc_ff9d",
    0xFFFC: "error_code_desc_fffc",
}
...
_STR: dict[str, dict[str, str]] = {
    "en": {
        ...
        "offline_message": (
            "Tablet **{device_name}** has not connected for over **{hours}** h. "
            "Check whether the battery ran out or Wi‑Fi was lost."
        ),
        "health_check_title": "Health check — {device_name}",
        "health_check_ok": (
            "No issues reported by VSS (**GET /api/orphans**). The WebKit session looks healthy."
        ),
        "health_check_problem": (
            "VSS reported a session problem (**GET /api/orphans**):\n\n{error}"
        ),
        "error_code_title": "Device error — {device_name}",
        "error_code_message": (
            "Tablet **{device_name}** reported VSS error code **{code}**.\n\n{description}"
        ),
        "error_code_connect_reason": "Last connect reason: **{reason}**",
        "error_code_desc_2": (
            "Wi‑Fi: scan requested while already associated with an access point."
        ),
        "error_code_desc_6": (
            "Wi‑Fi: no association with the access point (tablet is not joined to the network). "
            "Check signal, router idle disconnect, or 2.4 GHz WPA2 settings."
        ),
        "error_code_desc_3": (
            "Wi‑Fi: access point not found. Check SSID, range, and that the AP broadcasts 2.4 GHz."
        ),
        "error_code_desc_4": (
            "Wi‑Fi: wrong password for WEP network (Joan needs WPA2‑PSK)."
        ),
        "error_code_desc_5": (
            "Wi‑Fi: invalid band — use 2.4 GHz, not 5 GHz only."
        ),
        "error_code_desc_8": (
            "Wi‑Fi: deauthentication from the access point. Check password and AP security (WPA2‑PSK)."
        ),
        "error_code_desc_9": (
            "Wi‑Fi: failed to associate during join — wrong password, security mode, or AP settings."
        ),
        "error_code_desc_10": (
            "Wi‑Fi: invalid channel — router/AP channel may be unsupported."
        ),
        "error_code_desc_14": (
            "Wi‑Fi: authentication failed during join, or AP disappeared after scan."
        ),
        "error_code_desc_15": (
            "Wi‑Fi: missed beacon from AP during join — weak signal or AP sleep."
        ),
        "error_code_desc_25": (
            "Wi‑Fi: rejoin failure / AP not found. Often range, roaming, or wrong credentials."
        ),
        "error_code_desc_26": (
            "Wi‑Fi: frequency not supported by the module."
        ),
        "error_code_desc_32": (
            "Wi‑Fi: unable to join the network."
        ),
        "error_code_desc_37": (
            "Wi‑Fi: command issued while re‑join is still in progress — wait and retry."
        ),
        "error_code_desc_38": (
            "Wi‑Fi: invalid parameters in the join command."
        ),
        "error_code_desc_40": (
            "Wi‑Fi: wrong PSK length (password too short or longer than allowed)."
        ),
        "error_code_desc_41": (
            "Wi‑Fi: wrong PSK length (Joan: password max ~31 characters)."
        ),
        "error_code_desc_68": (
            "Wi‑Fi: deauth due to missed beacons — move tablet closer to the AP."
        ),
        "error_code_desc_73": (
            "Wi‑Fi: association timeout — signal, password, or AP load."
        ),
        "error_code_desc_81": (
            "Wi‑Fi: 4‑way handshake failed — usually wrong Wi‑Fi password."
        ),
        "error_code_desc_bb38": (
            "TCP: tablet cannot reach the VSS server socket. Check HA/VSS on port **11113**, firewall, and ping from the tablet VLAN."
        ),
        "error_code_desc_ff87": (
            "TCP: connection to VSS server failed (non‑existent or unreachable socket). Verify **11113** and server IP in Joan configurator."
        ),
        "error_code_desc_ff9c": (
            "DHCP handshake failure. Check DHCP pool, Wi‑Fi signal, and router rules for sleeping clients."
        ),
        "error_code_desc_ff9d": (
            "DHCP lease expired — renew IP or check router DHCP settings."
        ),
        "error_code_desc_fffc": (
            "IP configuration failed — DHCP/static settings on tablet or router."
        ),
        "error_code_desc_unknown": (
            "No built‑in description for this code. See "
            "[Joan Wi‑Fi error codes](https://support.getjoan.com/knowledge/wi-fi-error-codes) "
            "or VSS logs; Configurator over USB gives more detail."
        ),
        ...
        "self_test_title": "Visionect self-test",
        "self_test_status": "Visionect self-test: {status}",
        "self_test_status_pass": "PASS",
        "self_test_status_warn": "WARN",
        "self_test_vss_health": "VSS health: {status}",
        "self_test_devices_selected": "Devices selected: {count}",
        "self_test_refresh_ok": "Device data refresh OK: {ok}/{total}",
        "self_test_restart_ok": "Session restart OK: {ok}/{total}",
        "self_test_metrics_header": "Battery guard metrics:",
        "self_test_metrics_line": (
            "- attempts={attempts}, put_success={put_success}, "
            "skip_duplicate={skip_duplicate}, skip_guard={skip_guard}, skip_unchanged={skip_unchanged}"
        ),
        "self_test_metrics_hint": (
            "Metric explanation: attempts=all URL-change attempts; put_success=actual writes sent to VSS; "
            "skip_duplicate=same URL repeated too fast; skip_guard=different URL blocked by guard interval; "
            "skip_unchanged=current VSS URL already matched requested URL. "
            "All zeros are normal when this test does not trigger any content URL change."
        ),
        "self_test_failed_uuids": "Failed UUIDs: {uuids}",
        ...
        "diag_group_refresh_profile": "HA adaptive refresh (Joan)",
        "diag_refresh_profile_this": "This tablet's profile: **{profile}** (eco / normal / alert).",
        "diag_refresh_profile_effective": (
            "Integration-wide dominant VSS HTTP cache profile: **{effective}** (strictest across tablets: alert > normal > eco)."
        ),
        "diag_refresh_profile_hint": (
            "Use these when judging how aggressively Home Assistant polls VSS and how fresh in-memory REST cache may be."
        ),
        "ai_roster_refresh_header": "[Home Assistant — adaptive refresh profiles (Visionect Joan)]",
        "ai_roster_shared_cache_effective": (
            "Dominant shared VSS HTTP cache profile for this integration entry: **{effective}** "
            "(strictest of all configured tablets: alert > normal > eco)."
        ),
        "ai_roster_refresh_hint": "Each device block lists this tablet's profile on the line immediately after UUID.",
        "ai_device_refresh_profile": "HA adaptive refresh profile: **{profile}**",
        ...
        "refresh_profile_notification_title": "Visionect: refresh profile changed ({device_name})",
        "refresh_profile_notification_body_eco": (
            "Tablet `{device_name}` is now in **eco** mode.\n\n"
            "- Behavior: limits frequent content changes and increases slideshow minimum interval.\n"
            "- Benefit: fewer Wi-Fi/e-ink wakeups.\n"
            "- Result: best battery life, but slower reaction to rapid changes.\n"
            "- Screen cache: longer per-screen TTL (fewer re-renders/API calls).\n\n"
            "**This tablet only:** the profile applies **only** to `{device_name}` (HA↔VSS polling, session-fetch policy, live camera throttling). "
            "Other tablets keep **their own** profiles.\n\n"
            "**Shared VSS HTTP client (one per integration entry):** REST responses are cached in memory; **that** cache’s TTL uses the **strictest** "
            "profile among **all** Joan tablets on this entry (**alert** > **normal** > **eco**). "
            "This does **not** replace per-tablet settings—it only caps how long **shared** API responses may be reused for this connection.\n\n"
            "**HA live view (camera)** for this tablet: screenshot requests are throttled (~**30 s**); "
            "the last successful image is reused between fetches."
        ),
        "refresh_profile_notification_body_normal": (
            "Tablet `{device_name}` is now in **normal** mode.\n\n"
            "- Behavior: balanced refresh strategy.\n"
            "- Benefit: good compromise between speed and battery protection.\n"
            "- Result: moderate battery usage.\n"
            "- Screen cache: baseline per-screen TTL.\n\n"
            "**This tablet only:** the profile applies **only** to `{device_name}` (HA↔VSS polling, session-fetch policy, live camera throttling). "
            "Other tablets keep **their own** profiles.\n\n"
            "**Shared VSS HTTP client (one per integration entry):** in-memory REST cache TTL uses the **strictest** profile among **all** tablets here "
            "(**alert** > **normal** > **eco**). This does **not** replace per-tablet settings.\n\n"
            "**HA live view (camera)** for this tablet: screenshot requests are throttled (~**12 s**); "
            "the last successful image is reused between fetches."
        ),
        "refresh_profile_notification_body_alert": (
            "Tablet `{device_name}` is now in **alert** mode.\n\n"
            "- Behavior: allows more frequent and faster content changes.\n"
            "- Benefit: fastest dashboard reaction.\n"
            "- Result: higher battery usage.\n"
            "- Screen cache: shorter per-screen TTL (faster data refresh).\n\n"
            "**This tablet only:** the profile applies **only** to `{device_name}` (HA↔VSS polling, session-fetch policy, live camera throttling). "
            "Other tablets keep **their own** profiles.\n\n"
            "**Shared VSS HTTP client (one per integration entry):** while **any** tablet uses **alert**, the in-memory REST cache uses shorter TTLs "
            "(fresher data, more HA↔VSS traffic) for this connection. This does **not** replace per-tablet settings.\n\n"
            "**HA live view (camera)** for this tablet: screenshot requests are throttled (~**6 s**); "
            "the last successful image is reused between fetches."
        ),
    ...
    "pl": {
        ...
        "offline_message": (
            "Tablet **{device_name}** nie połączył się od ponad **{hours}** h. "
            "Sprawdź, czy nie rozładowała się bateria lub czy nie stracił zasięgu Wi‑Fi."
        ),
        "health_check_title": "Kontrola stanu — {device_name}",
        "health_check_ok": (
            "Brak problemów wg VSS (**GET /api/orphans**). Sesja WebKit wygląda na poprawną."
        ),
        "health_check_problem": (
            "VSS zgłosił problem sesji (**GET /api/orphans**):\n\n{error}"
        ),
        "error_code_title": "Błąd urządzenia — {device_name}",
        "error_code_message": (
            "Tablet **{device_name}** zgłosił kod błędu VSS **{code}**.\n\n{description}"
        ),
        "error_code_connect_reason": "Ostatnia przyczyna połączenia: **{reason}**",
        "error_code_desc_2": "Wi‑Fi: skanowanie przy już aktywnym połączeniu z AP.",
        "error_code_desc_6": (
            "Wi‑Fi: brak skojarzenia z punktem dostępu (tablet nie jest podłączony do sieci). "
            "Sprawdź zasięg, rozłączanie idle w routerze i WPA2 na 2,4 GHz."
        ),
        "error_code_desc_3": (
            "Wi‑Fi: nie znaleziono punktu dostępu. Sprawdź SSID, zasięg i czy AP nadaje 2,4 GHz."
        ),
        "error_code_desc_4": "Wi‑Fi: złe hasło WEP (Joan wymaga WPA2‑PSK).",
        "error_code_desc_5": "Wi‑Fi: zła częstotliwość — użyj 2,4 GHz, nie tylko 5 GHz.",
        "error_code_desc_8": (
            "Wi‑Fi: rozłączenie (deauth) z punktu dostępu. Sprawdź hasło i zabezpieczenia AP (WPA2‑PSK)."
        ),
        "error_code_desc_9": "Wi‑Fi: nie udało się skojarzyć — hasło, tryb zabezpieczeń lub ustawienia AP.",
        "error_code_desc_10": "Wi‑Fi: nieprawidłowy kanał routera/AP.",
        "error_code_desc_14": "Wi‑Fi: błąd uwierzytelnienia przy join lub AP zniknął po skanie.",
        "error_code_desc_15": "Wi‑Fi: brak beaconów AP przy join — słaby sygnał lub uśpiony router.",
        "error_code_desc_25": (
            "Wi‑Fi: błąd ponownego dołączenia / brak AP. Często zasięg, roaming lub złe dane logowania."
        ),
        "error_code_desc_26": "Wi‑Fi: częstotliwość nieobsługiwana przez moduł.",
        "error_code_desc_32": "Wi‑Fi: nie można dołączyć do sieci.",
        "error_code_desc_37": "Wi‑Fi: polecenie w trakcie re‑join — odczekaj i spróbuj ponownie.",
        "error_code_desc_38": "Wi‑Fi: nieprawidłowe parametry polecenia join.",
        "error_code_desc_40": (
            "Wi‑Fi: zła długość hasła PSK (za krótkie lub za długie)."
        ),
        "error_code_desc_41": (
            "Wi‑Fi: zła długość hasła PSK (Joan: hasło max ~31 znaków)."
        ),
        "error_code_desc_68": "Wi‑Fi: deauth przez brak beaconów — zbliż tablet do AP.",
        "error_code_desc_73": "Wi‑Fi: timeout skojarzenia — sygnał, hasło lub obciążenie AP.",
        "error_code_desc_81": "Wi‑Fi: błąd 4‑way handshake — zwykle złe hasło Wi‑Fi.",
        "error_code_desc_bb38": (
            "TCP: tablet nie dociera do gniazda VSS. Sprawdź HA/VSS na porcie **11113**, firewall i ping z sieci tabletu."
        ),
        "error_code_desc_ff87": (
            "TCP: brak połączenia z serwerem VSS. Zweryfikuj **11113** i adres serwera w Joan Configurator."
        ),
        "error_code_desc_ff9c": (
            "Błąd DHCP. Sprawdź pulę adresów, sygnał Wi‑Fi i reguły routera dla uśpionych klientów."
        ),
        "error_code_desc_ff9d": "Wygasła dzierżawa DHCP — odśwież IP lub ustawienia DHCP routera.",
        "error_code_desc_fffc": "Błąd konfiguracji IP — DHCP/statyczne ustawienia tabletu lub routera.",
        "error_code_desc_unknown": (
            "Brak opisu tego kodu w integracji. Zobacz "
            "[kody błędów Wi‑Fi Joan](https://support.getjoan.com/knowledge/wi-fi-error-codes) "
            "lub logi VSS; szczegóły da Configurator przez USB."
        ),
        ...
        "self_test_status_pass": "PASS",
        "self_test_status_warn": "WARN",
        "self_test_vss_health": "Stan VSS: {status}",
        "self_test_devices_selected": "Wybrane urządzenia: {count}",
        "self_test_refresh_ok": "Odświeżenie danych urządzeń OK: {ok}/{total}",
        "self_test_restart_ok": "Restart sesji OK: {ok}/{total}",
        "self_test_metrics_header": "Metryki battery guard:",
        "self_test_metrics_line": (
            "- attempts={attempts}, put_success={put_success}, "
            "skip_duplicate={skip_duplicate}, skip_guard={skip_guard}, skip_unchanged={skip_unchanged}"
        ),
        "self_test_metrics_hint": (
            "Znaczenie metryk: attempts=wszystkie próby zmiany URL; put_success=rzeczywiste zapisy wysłane do VSS; "
            "skip_duplicate=pominięto ten sam URL wysłany zbyt szybko; "
            "skip_guard=pominięto inny URL, bo zadziałał interwał ochronny; "
            "skip_unchanged=na VSS był już dokładnie ten sam URL. "
            "Wartości 0 są normalne, gdy test nie wykonuje zmian URL treści."
        ),
        "self_test_failed_uuids": "Nieudane UUID: {uuids}",
        ...
        "diag_group_refresh_profile": "Adaptacyjne odświeżanie HA (Joan)",
        "diag_refresh_profile_this": "Profil **tego** tabletu: **{profile}** (eco / normal / alert).",
        "diag_refresh_profile_effective": (
            "Dominujący profil wspólnego cache HTTP do VSS dla tego wpisu integracji: **{effective}** "
            "(najostrzejszy spośród zapisanych tabletów: alert > normal > eco)."
        ),
        "diag_refresh_profile_hint": (
            "Uwzględnij to przy ocenie częstotliwości odpytywania VSS i „świeżości” cache REST po stronie integracji."
        ),
        "ai_roster_refresh_header": "[Home Assistant — profile adaptacyjnego odświeżania (Visionect Joan)]",
        "ai_roster_shared_cache_effective": (
            "Dominujący wspólny profil cache HTTP do VSS dla tego wpisu integracji: **{effective}** "
            "(najostrzejszy spośród zapisanych tabletów: alert > normal > eco)."
        ),
        "ai_roster_refresh_hint": "Profil danego tabletu jest w jego bloku, w linii zaraz po UUID.",
        "ai_device_refresh_profile": "Profil adaptacyjnego odświeżania (HA): **{profile}**",
        ...
        "refresh_profile_notification_title": "Visionect: zmiana profilu odświeżania ({device_name})",
        "refresh_profile_notification_body_eco": (
            "Tablet `{device_name}` ustawiono na tryb **eco**.\n\n"
            "- Działanie: ogranicza częste zmiany treści i wydłuża minimalny czas slajdu.\n"
            "- Co daje: mniej wybudzeń Wi-Fi/ekranu e-ink.\n"
            "- Efekt: najdłuższa praca na baterii, ale wolniejsze reakcje na szybkie zmiany.\n"
            "- Cache ekranów: dłuższy TTL (mniej renderów i wywołań API).\n\n"
            "**Tylko ten tablet:** wybrany profil dotyczy **wyłącznie** `{device_name}` (m.in. polling HA↔VSS, polityka pobierania sesji, tempo podglądu kamery). "
            "Pozostałe tablety mają **swoje** niezależne profile.\n\n"
            "**Wspólny klient HTTP do VSS (jeden na ten wpis integracji):** odpowiedzi REST są cachowane w pamięci; **czas życia tego współdzielonego cache** "
            "wynika z **najostrzejszego** profilu spośród **wszystkich** tabletów Joan w tej konfiguracji (**alert** > **normal** > **eco**). "
            "To **nie** nadpisuje profilu per tablet — tylko ogranicza, jak długo **wspólne** odpowiedzi API mogą być ponownie użyte dla tego połączenia.\n\n"
            "**Podgląd w HA (encja kamery)** dla tego tabletu: minimalny odstęp między zrzutami ekranu ~**30 s**; "
            "między zapytaniami zwracany jest ostatni poprawny obraz."
        ),
        "refresh_profile_notification_body_normal": (
            "Tablet `{device_name}` ustawiono na tryb **normal**.\n\n"
            "- Działanie: zbalansowane odświeżanie.\n"
            "- Co daje: dobry kompromis między szybkością i oszczędzaniem energii.\n"
            "- Efekt: umiarkowane zużycie baterii.\n"
            "- Cache ekranów: bazowy TTL.\n\n"
            "**Tylko ten tablet:** profil dotyczy **wyłącznie** `{device_name}` (m.in. polling, sesja, kamera). "
            "Inne tablety zachowują **swoje** ustawienia.\n\n"
            "**Wspólny klient HTTP do VSS (jeden na ten wpis):** cache odpowiedzi REST w pamięci używa **najostrzejszego** profilu ze wszystkich tabletów "
            "(**alert** > **normal** > **eco**). To **nie** zmienia profilu per UUID — dotyczy tylko współdzielonego cache tego połączenia.\n\n"
            "**Podgląd w HA (encja kamery)** dla tego tabletu: minimalny odstęp między zrzutami ~**12 s**; "
            "między zapytaniami zwracany jest ostatni poprawny obraz."
        ),
        "refresh_profile_notification_body_alert": (
            "Tablet `{device_name}` ustawiono na tryb **alert**.\n\n"
            "- Działanie: pozwala na częstsze i szybsze zmiany treści.\n"
            "- Co daje: najszybsza reakcja dashboardu.\n"
            "- Efekt: wyższe zużycie baterii.\n"
            "- Cache ekranów: krótszy TTL (szybsze odświeżanie danych).\n\n"
            "**Tylko ten tablet:** profil dotyczy **wyłącznie** `{device_name}` (polling, sesja, kamera). "
            "Inne tablety mają **swoje** profile.\n\n"
            "**Wspólny klient HTTP do VSS (jeden na ten wpis):** gdy **którykolwiek** tablet ma **alert**, współdzielony cache REST ma krótsze TTL "
            "(świeższe dane, więcej HA↔VSS). To **nie** zastępuje profilu per tablet.\n\n"
            "**Podgląd w HA (encja kamery)** dla tego tabletu: minimalny odstęp między zrzutami ~**6 s**; "
            "między zapytaniami zwracany jest ostatni poprawny obraz."
        ),
    ...
    "de": {
        ...
        "offline_message": (
            "Tablet **{device_name}** war über **{hours}** h nicht verbunden. Prüfen Sie Akku und WLAN."
        ),
        "health_check_title": "Zustandsprüfung — {device_name}",
        "health_check_ok": (
            "Keine Probleme laut VSS (**GET /api/orphans**). Die WebKit-Sitzung wirkt in Ordnung."
        ),
        "health_check_problem": (
            "VSS meldet ein Sitzungsproblem (**GET /api/orphans**):\n\n{error}"
        ),
        "error_code_title": "Gerätefehler — {device_name}",
        "error_code_message": (
            "Tablet **{device_name}** meldet VSS-Fehlercode **{code}**.\n\n{description}"
        ),
        "error_code_connect_reason": "Letzter Verbindungsgrund: **{reason}**",
        "error_code_desc_6": "WLAN: keine Zuordnung zum Access Point (Tablet nicht im Netz). Signal/Router prüfen.",
        "error_code_desc_3": "WLAN: Access Point nicht gefunden. SSID, Reichweite, 2,4 GHz prüfen.",
        "error_code_desc_8": "WLAN: Deauth vom Access Point. Passwort und WPA2‑PSK prüfen.",
        "error_code_desc_25": "WLAN: Rejoin fehlgeschlagen / AP nicht gefunden.",
        "error_code_desc_40": "WLAN: falsche PSK-Länge.",
        "error_code_desc_41": "WLAN: falsche PSK-Länge (Joan: max. ~31 Zeichen).",
        "error_code_desc_bb38": "TCP: VSS-Socket nicht erreichbar. Port **11113**, Firewall prüfen.",
        "error_code_desc_ff87": "TCP: Verbindung zum VSS-Server fehlgeschlagen (**11113**).",
        "error_code_desc_ff9c": "DHCP-Handshake fehlgeschlagen.",
        "error_code_desc_ff9d": "DHCP-Lease abgelaufen.",
        "error_code_desc_fffc": "IP-Konfiguration fehlgeschlagen.",
        "error_code_desc_unknown": "Keine Beschreibung für diesen Code in der Integration. Joan-WLAN-Codes oder VSS-Logs prüfen.",
        ...
        "diag_online_check_label": "Device online check",
        "diag_ollama_hint": (
            "Wenn Ollama aktiv ist, liest die KI die Abschnitte in dieser Reihenfolge: "
            "Kernprüfungen → erweiterte Prüfungen → Erkenntnisse, danach eine kurze Sanierungszusammenfassung."
        ),
        "diag_group_refresh_profile": "Adaptives Aktualisieren in HA (Joan)",
        "diag_refresh_profile_this": "Profil **dieses** Tablets: **{profile}** (eco / normal / alert).",
        "diag_refresh_profile_effective": (
            "Dominantes gemeinsames VSS-HTTP-Cacheprofil für diesen Integrationseintrag: **{effective}** "
            "(strengstes unter allen konfigurierten Tablets: alert > normal > eco)."
        ),
        "diag_refresh_profile_hint": (
            "Nutzen Sie diese Angaben zur Einschätzung, wie aggressiv Home Assistant den VSS abfragt und wie frisch der REST-Cache sein kann."
        ),
        "ai_roster_refresh_header": "[Home Assistant — adaptive Aktualisierungsprofile (Visionect Joan)]",
        "ai_roster_shared_cache_effective": (
            "Dominantes gemeinsames VSS-HTTP-Cacheprofil für diesen Integrationseintrag: **{effective}** "
            "(strengstes unter allen konfigurierten Tablets: alert > normal > eco)."
        ),
        "ai_roster_refresh_hint": "Das Profil jedes Tablets steht in seinem Geräteblock in der Zeile direkt unter der UUID.",
        "ai_device_refresh_profile": "Adaptives HA-Aktualisierungsprofil: **{profile}**",
        ...
        "refresh_profile_notification_title": "Visionect: Aktualisierungsprofil geändert ({device_name})",
        "refresh_profile_notification_body_eco": (
            "Tablet `{device_name}` ist jetzt im Modus **eco**.\n\n"
            "- Verhalten: begrenzt häufige Inhaltswechsel und erhöht das minimale Slideshow-Intervall.\n"
            "- Vorteil: weniger WLAN-/E-Ink-Aufweckvorgänge.\n"
            "- Ergebnis: beste Akkulaufzeit, aber langsamere Reaktion auf schnelle Änderungen.\n"
            "- Bildschirm-Cache: längere TTL (weniger Re-Render und API-Aufrufe).\n\n"
            "**Nur dieses Tablet:** Das Profil gilt **ausschließlich** für `{device_name}` (HA↔VSS-Polling, Session-Fetch, Live-Kamera). "
            "Andere Tablets behalten **ihre eigenen** Profile.\n\n"
            "**Gemeinsamer VSS-HTTP-Client (einer pro Integrationseintrag):** REST-Antworten werden im Speicher gecacht; die **TTL dieses gemeinsamen Caches** "
            "folgt dem **strengsten** Profil **aller** Joan-Tablets in diesem Eintrag (**alert** > **normal** > **eco**). "
            "Das ersetzt **keine** pro-Tablet-Einstellungen—es begrenzt nur, wie lange **gemeinsame** API-Antworten für diese Verbindung wiederverwendet werden dürfen.\n\n"
            "**HA-Livebild (Kamera)** für dieses Tablet: Screenshots werden gedrosselt (~**30 s**); "
            "zwischen den Abfragen wird das letzte gültige Bild wiederverwendet."
        ),
        "refresh_profile_notification_body_normal": (
            "Tablet `{device_name}` ist jetzt im Modus **normal**.\n\n"
            "- Verhalten: ausgewogene Aktualisierungsstrategie.\n"
            "- Vorteil: guter Kompromiss zwischen Geschwindigkeit und Akkuschutz.\n"
            "- Ergebnis: moderater Akkuverbrauch.\n"
            "- Bildschirm-Cache: Standard-TTL.\n\n"
            "**Nur dieses Tablet:** Das Profil gilt **nur** für `{device_name}` (Polling, Session, Kamera). "
            "Andere Tablets behalten **eigene** Profile.\n\n"
            "**Gemeinsamer VSS-HTTP-Client (einer pro Eintrag):** Der gemeinsame REST-Cache nutzt das **strengste** Profil aller Tablets (**alert** > **normal** > **eco**). "
            "Kein Ersatz für pro-Tablet-Profile.\n\n"
            "**HA-Livebild (Kamera)** für dieses Tablet: Screenshots ~**12 s** gedrosselt; "
            "letztes gültiges Bild wird zwischen den Abfragen wiederverwendet."
        ),
        "refresh_profile_notification_body_alert": (
            "Tablet `{device_name}` ist jetzt im Modus **alert**.\n\n"
            "- Verhalten: erlaubt häufigere und schnellere Inhaltswechsel.\n"
            "- Vorteil: schnellste Dashboard-Reaktion.\n"
            "- Ergebnis: höherer Akkuverbrauch.\n"
            "- Bildschirm-Cache: kürzere TTL (schnellere Datenaktualisierung).\n\n"
            "**Nur dieses Tablet:** Profil gilt **nur** für `{device_name}` (Polling, Session, Kamera). "
            "Andere Tablets: **eigene** Profile.\n\n"
            "**Gemeinsamer VSS-HTTP-Client:** Solange **ein** Tablet **alert** nutzt, bleibt der gemeinsame REST-Cache kürzer (mehr HA↔VSS). "
            "Ersetzt keine pro-Tablet-Einstellungen.\n\n"
            "**HA-Livebild (Kamera)** für dieses Tablet: Screenshots ~**6 s** gedrosselt; "
            "letztes gültiges Bild wird zwischen den Abfragen wiederverwendet."
        ),
    ...
    "fr": {
        ...
        "offline_message": (
            "La tablette **{device_name}** ne s’est pas connectée depuis plus de **{hours}** h. "
            "Vérifiez la batterie ou le Wi‑Fi."
        ),
        "health_check_title": "Contrôle d’état — {device_name}",
        "health_check_ok": (
            "Aucun problème signalé par le VSS (**GET /api/orphans**). La session WebKit semble saine."
        ),
        "health_check_problem": (
            "Le VSS signale un problème de session (**GET /api/orphans**) :\n\n{error}"
        ),
        "error_code_title": "Erreur appareil — {device_name}",
        "error_code_message": (
            "La tablette **{device_name}** signale le code d’erreur VSS **{code}**.\n\n{description}"
        ),
        "error_code_connect_reason": "Dernière raison de connexion : **{reason}**",
        "error_code_desc_6": "Wi‑Fi : pas d’association au point d’accès (tablette hors réseau).",
        "error_code_desc_3": "Wi‑Fi : point d’accès introuvable. Vérifier SSID, portée, 2,4 GHz.",
        "error_code_desc_8": "Wi‑Fi : déconnexion (deauth) par le point d’accès.",
        "error_code_desc_25": "Wi‑Fi : échec de reconnexion / AP introuvable.",
        "error_code_desc_40": "Wi‑Fi : longueur PSK incorrecte.",
        "error_code_desc_41": "Wi‑Fi : longueur PSK incorrecte (Joan : ~31 caractères max).",
        "error_code_desc_bb38": "TCP : socket VSS inaccessible. Port **11113**, pare-feu.",
        "error_code_desc_ff87": "TCP : connexion au serveur VSS échouée (**11113**).",
        "error_code_desc_ff9c": "Échec du DHCP.",
        "error_code_desc_ff9d": "Bail DHCP expiré.",
        "error_code_desc_fffc": "Échec de configuration IP.",
        "error_code_desc_unknown": "Pas de description intégrée pour ce code. Voir codes Wi‑Fi Joan ou journaux VSS.",
        ...
        "diag_online_check_label": "Device online check",
        "diag_ollama_hint": (
            "Avec Ollama activé, l’IA lit les sections dans cet ordre : "
            "contrôles de base → contrôles étendus → constats, puis un résumé d’actions concis."
        ),
        "diag_group_refresh_profile": "Rafraîchissement adaptatif HA (Joan)",
        "diag_refresh_profile_this": "Profil **de cette** tablette : **{profile}** (eco / normal / alert).",
        "diag_refresh_profile_effective": (
            "Profil de cache HTTP VSS partagé dominant pour cette entrée d’intégration : **{effective}** "
            "(le plus strict parmi les tablettes enregistrées : alert > normal > eco)."
        ),
        "diag_refresh_profile_hint": (
            "Tenez compte de ces valeurs pour juger la fréquence d’interrogation du VSS par Home Assistant et la fraîcheur du cache REST."
        ),
        "ai_roster_refresh_header": "[Home Assistant — profils de rafraîchissement adaptatif (Visionect Joan)]",
        "ai_roster_shared_cache_effective": (
            "Profil de cache HTTP VSS partagé dominant pour cette entrée d’intégration : **{effective}** "
            "(le plus strict parmi toutes les tablettes configurées : alert > normal > eco)."
        ),
        "ai_roster_refresh_hint": "Le profil de chaque tablette figure dans son bloc, sur la ligne juste après l’UUID.",
        "ai_device_refresh_profile": "Profil de rafraîchissement adaptatif (HA) : **{profile}**",
        ...
        "refresh_profile_notification_title": "Visionect : profil de rafraîchissement modifié ({device_name})",
        "refresh_profile_notification_body_eco": (
            "La tablette `{device_name}` est maintenant en mode **eco**.\n\n"
            "- Comportement : limite les changements de contenu fréquents et augmente l'intervalle minimal du diaporama.\n"
            "- Avantage : moins de réveils Wi-Fi/e-ink.\n"
            "- Résultat : meilleure autonomie, mais réaction plus lente aux changements rapides.\n"
            "- Cache écran : TTL plus long (moins de re-rendus et d'appels API).\n\n"
            "**Cette tablette uniquement :** le profil s’applique **uniquement** à `{device_name}` (polling HA↔VSS, politique de session, caméra live). "
            "Les autres tablettes gardent **leurs** profils.\n\n"
            "**Client HTTP VSS partagé (un par entrée d’intégration) :** les réponses REST sont mises en cache ; la **TTL de ce cache partagé** suit le profil le **plus strict** "
            "parmi **toutes** les Joan de cette entrée (**alert** > **normal** > **eco**). Cela **ne remplace** pas les réglages par tablette—cela limite seulement la durée de réutilisation des réponses **communes**.\n\n"
            "**Aperçu HA (caméra)** pour cette tablette : captures d'écran limitées (~**30 s**) ; "
            "la dernière image valide est réutilisée entre les requêtes."
        ),
        "refresh_profile_notification_body_normal": (
            "La tablette `{device_name}` est maintenant en mode **normal**.\n\n"
            "- Comportement : stratégie de rafraîchissement équilibrée.\n"
            "- Avantage : bon compromis entre rapidité et protection de la batterie.\n"
            "- Résultat : consommation de batterie modérée.\n"
            "- Cache écran : TTL de base.\n\n"
            "**Cette tablette uniquement :** le profil concerne **seulement** `{device_name}` (polling, session, caméra). "
            "Les autres : **profils séparés**.\n\n"
            "**Client HTTP partagé :** le cache REST suit le profil le **plus strict** de toutes les tablettes (**alert** > **normal** > **eco**). "
            "Sans remplacer les profils individuels.\n\n"
            "**Aperçu HA (caméra)** pour cette tablette : captures ~**12 s** ; dernière image valide réutilisée."
        ),
        "refresh_profile_notification_body_alert": (
            "La tablette `{device_name}` est maintenant en mode **alert**.\n\n"
            "- Comportement : permet des changements de contenu plus fréquents et plus rapides.\n"
            "- Avantage : réaction du tableau de bord la plus rapide.\n"
            "- Résultat : consommation de batterie plus élevée.\n"
            "- Cache écran : TTL plus court (données plus fraîches).\n\n"
            "**Cette tablette uniquement :** le profil vise **uniquement** `{device_name}` (polling, session, caméra). "
            "Les autres gardent **leurs** profils.\n\n"
            "**Client HTTP partagé :** dès qu’**une** tablette est en **alert**, le cache REST partagé est plus court (plus de trafic HA↔VSS). "
            "Ne remplace pas les profils par appareil.\n\n"
            "**Aperçu HA (caméra)** pour cette tablette : captures ~**6 s** ; dernière image valide réutilisée."
        ),
    ...
    "es": {
        ...
        "offline_message": (
            "La tablet **{device_name}** no se ha conectado en más de **{hours}** h. "
            "Compruebe batería o Wi‑Fi."
        ),
        "health_check_title": "Comprobación de estado — {device_name}",
        "health_check_ok": (
            "Sin problemas según VSS (**GET /api/orphans**). La sesión WebKit parece correcta."
        ),
        "health_check_problem": (
            "VSS informó de un problema de sesión (**GET /api/orphans**):\n\n{error}"
        ),
        "error_code_title": "Error del dispositivo — {device_name}",
        "error_code_message": (
            "La tableta **{device_name}** informó el código de error VSS **{code}**.\n\n{description}"
        ),
        "error_code_connect_reason": "Último motivo de conexión: **{reason}**",
        "error_code_desc_6": "Wi‑Fi: sin asociación al punto de acceso (tableta fuera de red).",
        "error_code_desc_3": "Wi‑Fi: punto de acceso no encontrado. Compruebe SSID, alcance y 2,4 GHz.",
        "error_code_desc_8": "Wi‑Fi: desautenticación del punto de acceso.",
        "error_code_desc_25": "Wi‑Fi: fallo al reincorporarse / AP no encontrado.",
        "error_code_desc_40": "Wi‑Fi: longitud PSK incorrecta.",
        "error_code_desc_41": "Wi‑Fi: longitud PSK incorrecta (Joan: máx. ~31 caracteres).",
        "error_code_desc_bb38": "TCP: socket VSS inaccesible. Puerto **11113**, firewall.",
        "error_code_desc_ff87": "TCP: fallo al conectar con el servidor VSS (**11113**).",
        "error_code_desc_ff9c": "Fallo de DHCP.",
        "error_code_desc_ff9d": "Concesión DHCP expirada.",
        "error_code_desc_fffc": "Fallo de configuración IP.",
        "error_code_desc_unknown": "Sin descripción integrada para este código. Vea códigos Wi‑Fi Joan o registros VSS.",
        ...
        "diag_online_check_label": "Device online check",
        "diag_ollama_hint": (
            "Con Ollama activada, la IA lee las secciones en este orden: "
            "comprobaciones básicas → ampliadas → hallazgos, y luego un resumen breve de remedio."
        ),
        "diag_group_refresh_profile": "Actualización adaptativa en HA (Joan)",
        "diag_refresh_profile_this": "Perfil **de esta** tableta: **{profile}** (eco / normal / alert).",
        "diag_refresh_profile_effective": (
            "Perfil dominante de caché HTTP compartido con VSS para esta entrada de integración: **{effective}** "
            "(el más estricto entre las tabletas configuradas: alert > normal > eco)."
        ),
        "diag_refresh_profile_hint": (
            "Úselos para valorar con qué frecuencia Home Assistant consulta al VSS y qué tan reciente puede estar la caché REST."
        ),
        "ai_roster_refresh_header": "[Home Assistant — perfiles de actualización adaptativa (Visionect Joan)]",
        "ai_roster_shared_cache_effective": (
            "Perfil dominante de caché HTTP compartido con VSS para esta entrada de integración: **{effective}** "
            "(el más estricto entre todas las tabletas configuradas: alert > normal > eco)."
        ),
        "ai_roster_refresh_hint": "El perfil de cada tableta está en su bloque, en la línea justo después del UUID.",
        "ai_device_refresh_profile": "Perfil de actualización adaptativa (HA): **{profile}**",
        ...
        "refresh_profile_notification_title": "Visionect: perfil de actualización cambiado ({device_name})",
        "refresh_profile_notification_body_eco": (
            "La tablet `{device_name}` ahora está en modo **eco**.\n\n"
            "- Comportamiento: limita cambios de contenido frecuentes y aumenta el intervalo mínimo del pase de diapositivas.\n"
            "- Beneficio: menos activaciones de Wi-Fi/e-ink.\n"
            "- Resultado: mejor autonomía, pero reacción más lenta ante cambios rápidos.\n"
            "- Caché de pantalla: TTL más largo (menos re-render y llamadas API).\n\n"
            "**Solo esta tableta:** el perfil aplica **únicamente** a `{device_name}` (polling HA↔VSS, política de sesión, cámara en vivo). "
            "Las demás mantienen **sus propios** perfiles.\n\n"
            "**Cliente HTTP compartido con VSS (uno por entrada de integración):** las respuestas REST se cachean en memoria; la **TTL de ese caché compartido** "
            "usa el perfil **más estricto** entre **todas** las Joan de esta entrada (**alert** > **normal** > **eco**). "
            "**No** sustituye los perfiles por tableta: solo limita cuánto tiempo pueden reutilizarse respuestas **compartidas** de esta conexión.\n\n"
            "**Vista en vivo en HA (cámara)** para esta tablet: capturas limitadas (~**30 s**); "
            "se reutiliza la última imagen válida entre peticiones."
        ),
        "refresh_profile_notification_body_normal": (
            "La tablet `{device_name}` ahora está en modo **normal**.\n\n"
            "- Comportamiento: estrategia de actualización equilibrada.\n"
            "- Beneficio: buen compromiso entre velocidad y protección de batería.\n"
            "- Resultado: consumo de batería moderado.\n"
            "- Caché de pantalla: TTL base.\n\n"
            "**Solo esta tableta:** el perfil es **solo** para `{device_name}` (polling, sesión, cámara). "
            "Las demás conservan **sus** perfiles.\n\n"
            "**Cliente HTTP compartido:** el caché REST usa el perfil **más estricto** de todas las tablets (**alert** > **normal** > **eco**). "
            "No reemplaza ajustes por dispositivo.\n\n"
            "**Vista en vivo en HA (cámara)** para esta tablet: capturas ~**12 s**; se reutiliza la última imagen válida."
        ),
        "refresh_profile_notification_body_alert": (
            "La tablet `{device_name}` ahora está en modo **alert**.\n\n"
            "- Comportamiento: permite cambios de contenido más frecuentes y rápidos.\n"
            "- Beneficio: respuesta más rápida del panel.\n"
            "- Resultado: mayor consumo de batería.\n"
            "- Caché de pantalla: TTL más corto (datos más frescos).\n\n"
            "**Solo esta tableta:** el perfil afecta **solo** a `{device_name}` (polling, sesión, cámara). "
            "Las demás mantienen **sus** perfiles.\n\n"
            "**Cliente HTTP compartido:** si **alguna** tableta está en **alert**, el caché REST compartido es más corto (más HA↔VSS). "
            "No sustituye perfiles por tableta.\n\n"
            "**Vista en vivo en HA (cámara)** para esta tablet: capturas ~**6 s**; se reutiliza la última imagen válida."
        ),
    ...
    "nl": {
        ...
        "offline_message": (
            "Tablet **{device_name}** is meer dan **{hours}** uur niet verbonden. "
            "Controleer batterij of Wi‑Fi."
        ),
        "health_check_title": "Statuscontrole — {device_name}",
        "health_check_ok": (
            "Geen problemen volgens VSS (**GET /api/orphans**). De WebKit-sessie lijkt in orde."
        ),
        "health_check_problem": (
            "VSS meldt een sessieprobleem (**GET /api/orphans**):\n\n{error}"
        ),
        "error_code_title": "Apparaatfout — {device_name}",
        "error_code_message": (
            "Tablet **{device_name}** meldde VSS-foutcode **{code}**.\n\n{description}"
        ),
        "error_code_connect_reason": "Laatste verbindingsreden: **{reason}**",
        "error_code_desc_6": "Wi‑Fi: geen associatie met access point (tablet niet op netwerk).",
        "error_code_desc_3": "Wi‑Fi: access point niet gevonden.",
        "error_code_desc_8": "Wi‑Fi: deauth door access point.",
        "error_code_desc_25": "Wi‑Fi: rejoin mislukt / AP niet gevonden.",
        "error_code_desc_40": "Wi‑Fi: onjuiste PSK-lengte.",
        "error_code_desc_41": "Wi‑Fi: onjuiste PSK-lengte (Joan: max. ~31 tekens).",
        "error_code_desc_bb38": "TCP: VSS-socket niet bereikbaar. Poort **11113**.",
        "error_code_desc_ff87": "TCP: verbinding met VSS-server mislukt (**11113**).",
        "error_code_desc_ff9c": "DHCP-handshake mislukt.",
        "error_code_desc_ff9d": "DHCP-lease verlopen.",
        "error_code_desc_fffc": "IP-configuratie mislukt.",
        "error_code_desc_unknown": "Geen ingebouwde beschrijving voor deze code.",
        ...
        "diag_online_check_label": "Device online check",
        "diag_ollama_hint": (
            "Met Ollama ingeschakeld leest de AI de secties in deze volgorde: "
            "kerncontroles → uitgebreide controles → bevindingen, daarna een korte herstelsamenvatting."
        ),
        "diag_group_refresh_profile": "Adaptief vernieuwen in HA (Joan)",
        "diag_refresh_profile_this": "Profiel **van deze** tablet: **{profile}** (eco / normal / alert).",
        "diag_refresh_profile_effective": (
            "Dominant gedeeld VSS-HTTP-cacheprofiel voor deze integratie‑invoer: **{effective}** "
            "(strengst van alle geconfigureerde tablets: alert > normal > eco)."
        ),
        "diag_refresh_profile_hint": (
            "Gebruik dit om in te schatten hoe agressief Home Assistant de VSS bevraagt en hoe vers de REST‑cache kan zijn."
        ),
        "ai_roster_refresh_header": "[Home Assistant — adaptieve vernieuwingsprofielen (Visionect Joan)]",
        "ai_roster_shared_cache_effective": (
            "Dominant gedeeld VSS-HTTP-cacheprofiel voor deze integratie‑invoer: **{effective}** "
            "(strengst van alle geconfigureerde tablets: alert > normal > eco)."
        ),
        "ai_roster_refresh_hint": "Het profiel van elke tablet staat in het apparaatblok op de regel direct onder de UUID.",
        "ai_device_refresh_profile": "Adaptief HA-vernieuwingsprofiel: **{profile}**",
        ...
        "refresh_profile_notification_title": "Visionect: vernieuwingsprofiel gewijzigd ({device_name})",
        "refresh_profile_notification_body_eco": (
            "Tablet `{device_name}` staat nu in **eco**-modus.\n\n"
            "- Gedrag: beperkt frequente inhoudswijzigingen en verhoogt het minimale slideshow-interval.\n"
            "- Voordeel: minder Wi-Fi/e-ink wake-ups.\n"
            "- Resultaat: beste batterijduur, maar tragere reactie op snelle veranderingen.\n"
            "- Schermcache: langere TTL (minder re-renders en API-calls).\n\n"
            "**Alleen deze tablet:** het profiel geldt **uitsluitend** voor `{device_name}` (HA↔VSS-polling, sessiebeleid, live camera). "
            "Andere tablets houden **hun eigen** profielen.\n\n"
            "**Gedeelde VSS-HTTP-client (één per integratie-invoer):** REST-antwoorden worden in geheugen gecached; de **TTL van die gedeelde cache** "
            "volgt het **strengste** profiel van **alle** Joan-tablets in deze invoer (**alert** > **normal** > **eco**). "
            "Dit **vervangt** geen per-tabletinstellingen—het beperkt alleen hoe lang **gedeelde** API-antwoorden voor deze verbinding hergebruikt mogen worden.\n\n"
            "**HA-liveweergave (camera)** voor deze tablet: screenshots beperkt (~**30 s**); "
            "laatste geldige afbeelding wordt tussen requests hergebruikt."
        ),
        "refresh_profile_notification_body_normal": (
            "Tablet `{device_name}` staat nu in **normal**-modus.\n\n"
            "- Gedrag: gebalanceerde vernieuwingsstrategie.\n"
            "- Voordeel: goed compromis tussen snelheid en batterijbescherming.\n"
            "- Resultaat: gemiddeld batterijverbruik.\n"
            "- Schermcache: standaard-TTL.\n\n"
            "**Alleen deze tablet:** profiel geldt **alleen** voor `{device_name}` (polling, sessie, camera). "
            "Andere tablets: **eigen** profielen.\n\n"
            "**Gedeelde HTTP-client:** REST-cache gebruikt het **strengste** profiel van alle tablets (**alert** > **normal** > **eco**). "
            "Vervangt geen per-apparaatinstellingen.\n\n"
            "**HA-liveweergave (camera)** voor deze tablet: screenshots ~**12 s**; laatste geldige afbeelding wordt hergebruikt."
        ),
        "refresh_profile_notification_body_alert": (
            "Tablet `{device_name}` staat nu in **alert**-modus.\n\n"
            "- Gedrag: staat frequentere en snellere inhoudswijzigingen toe.\n"
            "- Voordeel: snelste dashboardreactie.\n"
            "- Resultaat: hoger batterijverbruik.\n"
            "- Schermcache: kortere TTL (snellere data).\n\n"
            "**Alleen deze tablet:** profiel geldt **alleen** voor `{device_name}` (polling, sessie, camera). "
            "Andere: **eigen** profielen.\n\n"
            "**Gedeelde HTTP-client:** zolang **één** tablet **alert** gebruikt, blijft de gedeelde REST-cache korter (meer HA↔VSS). "
            "Vervangt geen per-tabletprofielen.\n\n"
            "**HA-liveweergave (camera)** voor deze tablet: screenshots ~**6 s**; laatste geldige afbeelding wordt hergebruikt."
        ),
    ...
    "cs": {
        ...
        "offline_message": (
            "Tablet **{device_name}** se nepřipojil déle než **{hours}** h. "
            "Zkontrolujte baterii nebo Wi‑Fi."
        ),
        "health_check_title": "Kontrola stavu — {device_name}",
        "health_check_ok": (
            "Žádné problémy podle VSS (**GET /api/orphans**). WebKit relace vypadá v pořádku."
        ),
        "health_check_problem": (
            "VSS hlásí problém relace (**GET /api/orphans**):\n\n{error}"
        ),
        "error_code_title": "Chyba zařízení — {device_name}",
        "error_code_message": (
            "Tablet **{device_name}** nahlásil chybový kód VSS **{code}**.\n\n{description}"
        ),
        "error_code_connect_reason": "Poslední důvod připojení: **{reason}**",
        "error_code_desc_6": "Wi‑Fi: žádná asociace s AP (tablet není v síti).",
        "error_code_desc_3": "Wi‑Fi: AP nenalezen.",
        "error_code_desc_8": "Wi‑Fi: deauth od AP.",
        "error_code_desc_25": "Wi‑Fi: selhání rejoin / AP nenalezen.",
        "error_code_desc_40": "Wi‑Fi: špatná délka PSK.",
        "error_code_desc_41": "Wi‑Fi: špatná délka PSK (Joan: max. ~31 znaků).",
        "error_code_desc_bb38": "TCP: VSS socket nedostupný. Port **11113**.",
        "error_code_desc_ff87": "TCP: připojení k VSS serveru selhalo (**11113**).",
        "error_code_desc_ff9c": "Selhání DHCP.",
        "error_code_desc_ff9d": "Vypršela DHCP lease.",
        "error_code_desc_fffc": "Selhala konfigurace IP.",
        "error_code_desc_unknown": "Pro tento kód není v integraci popis.",
        ...
        "diag_online_check_label": "Device online check",
        "diag_ollama_hint": (
            "Se zapnutou Ollamou AI čte sekce v tomto pořadí: "
            "základní kontroly → rozšířené kontroly → závěry, poté stručné shrnutí nápravy."
        ),
        "diag_group_refresh_profile": "Adaptivní obnovování v HA (Joan)",
        "diag_refresh_profile_this": "Profil **tohoto** tabletu: **{profile}** (eco / normal / alert).",
        "diag_refresh_profile_effective": (
            "Dominantní profil sdílené HTTP cache do VSS pro tento záznam integrace: **{effective}** "
            "(nejpřísnější ze všech nakonfigurovaných tabletů: alert > normal > eco)."
        ),
        "diag_refresh_profile_hint": (
            "Zohledněte to při odhadu frekvence dotazování HA na VSS a čerstvosti REST cache."
        ),
        "ai_roster_refresh_header": "[Home Assistant — profily adaptivního obnovování (Visionect Joan)]",
        "ai_roster_shared_cache_effective": (
            "Dominantní profil sdílené HTTP cache do VSS pro tento záznam integrace: **{effective}** "
            "(nejpřísnější ze všech nakonfigurovaných tabletů: alert > normal > eco)."
        ),
        "ai_roster_refresh_hint": "Profil daného tabletu je v jeho bloku na řádku hned za UUID.",
        "ai_device_refresh_profile": "Profil adaptivního obnovování (HA): **{profile}**",
        ...
        "refresh_profile_notification_title": "Visionect: profil obnovování změněn ({device_name})",
        "refresh_profile_notification_body_eco": (
            "Tablet `{device_name}` je nyní v režimu **eco**.\n\n"
            "- Chování: omezuje časté změny obsahu a prodlužuje minimální interval slideshow.\n"
            "- Přínos: méně probuzení Wi-Fi/e-ink.\n"
            "- Výsledek: nejlepší výdrž baterie, ale pomalejší reakce na rychlé změny.\n"
            "- Cache obrazovky: delší TTL (méně přerenderování a API volání).\n\n"
            "**Jen tento tablet:** profil platí **pouze** pro `{device_name}` (polling HA↔VSS, politika relace, živá kamera). "
            "Ostatní tablety mají **vlastní** profily.\n\n"
            "**Sdílený HTTP klient do VSS (jeden na tento záznam integrace):** odpovědi REST jsou v paměti cachované; **TTL té sdílené cache** "
            "vychází z **nejpřísnějšího** profilu mezi **všemi** Joan tablety v této konfiguraci (**alert** > **normal** > **eco**). "
            "**Nenahrazuje** profily per tablet — omezuje jen, jak dlouho lze **společné** odpovědi API znovu použít pro toto spojení.\n\n"
            "**Náhled v HA (entita kamery)** pro tento tablet: snímky omezeny (~**30 s**); "
            "mezi požadavky se vrací poslední platný obrázek."
        ),
        "refresh_profile_notification_body_normal": (
            "Tablet `{device_name}` je nyní v režimu **normal**.\n\n"
            "- Chování: vyvážená strategie obnovování.\n"
            "- Přínos: dobrý kompromis mezi rychlostí a ochranou baterie.\n"
            "- Výsledek: střední spotřeba baterie.\n"
            "- Cache obrazovky: základní TTL.\n\n"
            "**Jen tento tablet:** profil se týká **jen** `{device_name}` (polling, relace, kamera). "
            "Ostatní zůstávají **samostatně**.\n\n"
            "**Sdílený HTTP klient:** REST cache používá **nejpřísnější** profil ze všech tabletů (**alert** > **normal** > **eco**). "
            "Nemění nastavení per zařízení.\n\n"
            "**Náhled v HA (entita kamery)** pro tento tablet: snímky ~**12 s**; mezi požadavky poslední platný obrázek."
        ),
        "refresh_profile_notification_body_alert": (
            "Tablet `{device_name}` je nyní v režimu **alert**.\n\n"
            "- Chování: umožňuje častější a rychlejší změny obsahu.\n"
            "- Přínos: nejrychlejší reakce dashboardu.\n"
            "- Výsledek: vyšší spotřeba baterie.\n"
            "- Cache obrazovky: kratší TTL (čerstvější data).\n\n"
            "**Jen tento tablet:** profil platí **jen** pro `{device_name}` (polling, relace, kamera). "
            "Ostatní mají **vlastní** profily.\n\n"
            "**Sdílený HTTP klient:** při **jakémkoli** tabletu v **alert** má sdílená REST cache kratší TTL (více HA↔VSS). "
            "Nenahrazuje profily per tablet.\n\n"
            "**Náhled v HA (entita kamery)** pro tento tablet: snímky ~**6 s**; mezi požadavky poslední platný obrázek."
        ),
...
_OLLAMA_VSS_API_PL = (
    ...
    "• `visionect_joan.run_self_test`: szybki test łączności i sesji dla wybranych UUID (health VSS, "
    "odświeżenie danych urządzenia + restart sesji). To nie jest test końcówek użytkowych (np. webhooków stron), "
    "ale test ścieżki VSS/API i operacji sesyjnych.\n"
    "• Interpretacja metryk battery guard z self-testu: attempts=próby zmiany URL; put_success=zapisy URL wysłane do VSS; "
    "skip_duplicate=pominięte duplikaty; skip_guard=pominięte zmiany z powodu interwału ochronnego; "
    "skip_unchanged=pominięte bo URL już był ustawiony. Zera w każdym polu są poprawne, gdy test nie wykonywał zmian URL.\n"
    "• Integracja Home Assistant (Joan): per tablet ustawiasz profil adaptacyjnego odświeżania **eco** / **normal** / **alert** "
    "(interwały HA↔VSS, polityka sesji, dławienie podglądu kamery). Wspólny cache HTTP klienta VSS dla wpisu konfiguracyjnego "
    "ma TTL wg **najostrzejszego** profilu spośród tabletów (**alert** > **normal** > **eco**). W payloadzie mogą być te wartości "
    "(lista urządzeń, diagnostyka logów) — uwzględnij je przy ocenie częstotliwości zapytań i opóźnień danych.\n"
)
...
_OLLAMA_VSS_API_EN = (
    ...
    "• `visionect_joan.run_self_test`: quick connectivity/session sanity test for selected UUIDs (VSS health, "
    "device data refresh, plus session restart). It is not an end-to-end test of user-facing content/webhook flows; "
    "it validates VSS/API path and session operations.\n"
    "• Battery guard metrics from self-test: attempts=URL change attempts; put_success=URL writes sent to VSS; "
    "skip_duplicate=duplicate URL writes skipped; skip_guard=changes skipped by guard interval; "
    "skip_unchanged=skipped because URL already matched. All zeros are valid when no content URL changes were triggered.\n"
    "• Home Assistant (Joan) integration: each tablet can use an adaptive refresh profile **eco** / **normal** / **alert** "
    "(HA↔VSS polling, session policy, live camera throttling). The shared in-memory VSS REST cache for the config entry picks "
    "TTLs from the **strictest** profile across tablets (**alert** > **normal** > **eco**). Device rosters and log-analysis "
    "diagnostics may include these values—factor them in when reasoning about request frequency and data freshness.\n"
)
...
def vss_error_code_display(code_raw: str | int | None) -> str:
    """Human-readable ErrorCode for HA notifications."""
    if code_raw is None:
        return "?"
    raw = str(code_raw).strip()
    if not raw:
        return "?"
    if raw.lower().startswith("0x"):
        return raw
    norm = vss_error_code_normalize(code_raw)
    if norm is None:
        return raw
    if norm <= 0xFFFF:
        return f"{norm} (0x{norm:04X})"
    return str(norm)


def vss_error_code_description(lang: str, code_num: int) -> str:
    """Localized hint for a normalized VSS/Joan error code."""
    key = _VSS_ERROR_DESC_KEYS.get(code_num, "error_code_desc_unknown")
    return ntr(lang, key)
