...
async def async_push_url_from_service(
    ...
    *,
    ...
    # Eco: same path (ignoring ?cb=) → skip. wake_tablet / Alert → still PUT so
    # the cache-buster from set_url can force WebKit to reload changed content.
    if current and urls_equivalent(api, current, url) and not force:
        _LOGGER.info(
            "Eco battery save: skipped set_url for %s (same page path, no VSS PUT). "
            "Use wake_tablet: true to reload content, or reload_url for stubborn cache.",
    ...
    if (
        ...
        _LOGGER.info(
            "Eco battery save: guard blocked URL change for %s (wait before next write). "
            "Use wake_tablet: true only if the tablet must update now.",
            normalize_device_uuid(device_uuid),
        )
...
def event_status_from_push_result(result: PushResult) -> str:
    """Map push result to automation status.

    skipped_* → status \"skipped\" (not success) so automations do not treat
    battery saves as a completed screen update and spam force_refresh.
    """
    if result == "success":
        return "success"
    if result in ("skipped_unchanged", "skipped_guard"):
        return "skipped"
    return "failure"
