...
def _host_display_ip(host: str) -> str:
    return host.replace("http://", "").replace("https://", "").split(":")[0]


def _entry_title_from_host(host: str, device_count: int = 0) -> str:
    title = f"Visionect Joan ({_host_display_ip(host)})"
    if device_count > 0:
...
def _merge_credentials(
    ...
    *,
    keep_secrets_when_blank: bool = False,
) -> Dict[str, Any]:
    """Build config entry data; blank password/secret keeps previous value when requested."""
    merged = {**base_data, CONF_HOST: user_input[CONF_HOST]}
    for key in _CREDENTIAL_KEYS:
        value = user_input.get(key)
        if value is None:
            continue
        text = str(value).strip()
        if not text and keep_secrets_when_blank and key in (CONF_PASSWORD, CONF_API_SECRET):
            continue
        if text or not keep_secrets_when_blank:
            merged[key] = text if text else value
    return merged
...
class VisionectJoanConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def _async_validate_server(
        ...
        """Test VSS connectivity; return (errors, device_count)."""
        errors: Dict[str, str] = {}
        try:
            api = VisionectAPI(
                self.hass,
                connection_data[CONF_HOST],
                connection_data.get(CONF_USERNAME),
                connection_data.get(CONF_PASSWORD),
                connection_data.get(CONF_API_KEY),
                connection_data.get(CONF_API_SECRET),
            )
            if not await api.async_test_authentication():
                return {"base": "auth_failed"}, None
            devices = await api.async_get_all_devices()
            return {}, len(devices or [])
        except Exception as ex:
            _LOGGER.error("Error during VSS connection test: %s", str(ex))
        ...
        return vol.Schema(
            {
                ...
                vol.Optional(
                    CONF_PASSWORD,
                    default=defaults.get(CONF_PASSWORD, "") if prefill_secrets else "",
                ): str,
                vol.Optional(
                    CONF_API_KEY,
                ...
                vol.Optional(
                    CONF_API_SECRET,
                    default=defaults.get(CONF_API_SECRET, "") if prefill_secrets else "",
                ): str,
            }
        )
    ...
    async def async_step_user(self, user_input=None):
        ...
        if user_input is not None:
            ...
            errors, device_count = await self._async_validate_server(user_input)
            if not errors and device_count is not None:
                return self.async_create_entry(
                    title=_entry_title_from_host(user_input[CONF_HOST], device_count),
                    data=user_input,
                )
    ...
    async def async_step_reconfigure(self, user_input=None):
        """Change VSS backend URL/port (e.g. after moving the add-on off :8081)."""
        errors: Dict[str, str] = {}
        reconfigure_entry = self._get_reconfigure_entry()
        current = reconfigure_entry.data

        if user_input is not None:
            new_data = _merge_credentials(
                current, user_input, keep_secrets_when_blank=True
            )
            await self.async_set_unique_id(new_data[CONF_HOST])
            self._abort_if_unique_id_configured(updates=reconfigure_entry)

            errors, device_count = await self._async_validate_server(new_data)
            if not errors and device_count is not None:
                return self.async_update_reload_and_abort(
                    reconfigure_entry,
                    unique_id=new_data[CONF_HOST],
                    data=new_data,
                    title=_entry_title_from_host(new_data[CONF_HOST], device_count),
                )
...
class VisionectJoanOptionsFlowHandler(config_entries.OptionsFlow):
    ...
    async def async_step_notifications(self, user_input=None):
        """Notifications: battery and VSS error codes."""
        return self.async_show_menu(
    ...
    async def async_step_error_code_alerts(self, user_input=None):
        """Persistent HA notification when VSS reports a non-zero ErrorCode."""
        if user_input is not None:
            merged = {**self._entry.options, **user_input}
            return self.async_create_entry(title="", data=merged)
        ...
        schema = vol.Schema(
            {
                vol.Optional(
                    CONF_ERROR_CODE_HA_NOTIFICATION_ENABLED,
                    default=bool(
                        self._entry.options.get(CONF_ERROR_CODE_HA_NOTIFICATION_ENABLED, True)
                    ),
                ): selector.BooleanSelector(),
            }
        )
        return self.async_show_form(step_id="error_code_alerts", data_schema=schema)

    async def async_step_offline_alerts(self, user_input=None):
        """Persistent HA notification when a tablet stays offline."""
        if user_input is not None:
            merged = {**self._entry.options, **user_input}
            merged[CONF_OFFLINE_THRESHOLD_HOURS] = int(
                merged[CONF_OFFLINE_THRESHOLD_HOURS]
            )
            return self.async_create_entry(title="", data=merged)

        try:
            thr_i = int(self._entry.options.get(CONF_OFFLINE_THRESHOLD_HOURS, 4))
        except (TypeError, ValueError):
            thr_i = 4
        if thr_i not in OFFLINE_THRESHOLD_HOUR_CHOICES:
            thr_i = 4
        thr_s = str(thr_i)
        schema = vol.Schema(
            {
                vol.Optional(
                    CONF_OFFLINE_HA_NOTIFICATION_ENABLED,
                    default=bool(
                        self._entry.options.get(CONF_OFFLINE_HA_NOTIFICATION_ENABLED, True)
                    ),
                ): selector.BooleanSelector(),
                vol.Optional(
                    CONF_OFFLINE_THRESHOLD_HOURS,
        ...
        return self.async_show_form(step_id="offline_alerts", data_schema=schema)

    async def async_step_health_alerts(self, user_input=None):
        """Automatic HA notification when VSS reports orphan / session problems."""
        if user_input is not None:
            merged = {**self._entry.options, **user_input}
            return self.async_create_entry(title="", data=merged)
        ...
        schema = vol.Schema(
            {
                vol.Optional(
                    CONF_HEALTH_HA_NOTIFICATION_ENABLED,
                    default=bool(
                        self._entry.options.get(CONF_HEALTH_HA_NOTIFICATION_ENABLED, False)
                    ),
                ): selector.BooleanSelector(),
            }
        )
        return self.async_show_form(step_id="health_alerts", data_schema=schema)
