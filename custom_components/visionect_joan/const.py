...
CONF_BATTERY_GUARD_INTERVAL_SEC = "battery_guard_interval_sec"

# HA persistent notifications when VSS Status.ErrorCode is non-zero
CONF_ERROR_CODE_HA_NOTIFICATION_ENABLED = "error_code_ha_notification_enabled"

# HA notification when tablet stays offline longer than threshold
CONF_OFFLINE_HA_NOTIFICATION_ENABLED = "offline_ha_notification_enabled"
CONF_OFFLINE_THRESHOLD_HOURS = "offline_threshold_hours"

# HA notification when VSS reports orphan / session problem (automatic poll)
CONF_HEALTH_HA_NOTIFICATION_ENABLED = "health_ha_notification_enabled"

OFFLINE_THRESHOLD_HOUR_CHOICES = (1, 4, 24)
...
VSS_ERROR_CODE_OK_STATE = "ok"


def vss_error_code_is_nonzero(code_raw: str | int | None) -> bool:
    """True when VSS Status.ErrorCode indicates an active device error."""
    if code_raw is None:
        return False
    t = str(code_raw).strip().lower()
    if not t or t in UNKNOWN_STRINGS or t in ("0", "0x0", "0x00", "00"):
        return False
...
def vss_error_code_sensor_state(code_raw: str | int | None) -> str | None:
    """Map VSS ErrorCode to a HA-friendly sensor state (ok / code / unknown)."""
    if code_raw is None:
        return None
    raw = str(code_raw).strip()
    if not raw or raw.lower() in UNKNOWN_STRINGS:
        return None
    if not vss_error_code_is_nonzero(raw):
        return VSS_ERROR_CODE_OK_STATE
    return raw


def vss_error_code_normalize(code_raw: str | int | None) -> int | None:
    """Parse VSS ErrorCode (decimal or 0x hex) to an integer for lookup tables."""
    if code_raw is None:
        return None
    t = str(code_raw).strip().lower()
    if not t or t in UNKNOWN_STRINGS:
        return None
    if t.startswith("0x"):
        try:
            return int(t, 16)
...
# VSS GUI/API: State bywa online|offline oraz sleeping|charging (ładowanie ≠ rozłączenie).
VSS_CONNECTED_STATES = frozenset({"1", "online", "2", "sleeping", "charging"})


def is_vss_device_connected(state: object | None) -> bool:
    """True when VSS State means the tablet still has a live link (not offline)."""
    if state is None:
        return False
    return str(state).strip().lower() in VSS_CONNECTED_STATES

MODEL_JOAN6 = "Joan 6"

SCAN_INTERVAL = timedelta(minutes=15)
POLL_INTERVAL_ECO = timedelta(minutes=20)
POLL_INTERVAL_NORMAL = timedelta(minutes=15)
POLL_INTERVAL_ALERT = timedelta(minutes=5)
SESSION_REFRESH_EVERY_TICKS_ECO = 6
SESSION_REFRESH_EVERY_TICKS_NORMAL = 4
SESSION_REFRESH_EVERY_TICKS_ALERT = 2
BATTERY_VOLTAGE_DIVIDER = 1000

# Endpointy API Visionect (kolekcje z / na końcu; szczegóły {uuid} bez — zgodnie z VSS na LAN)
API_PING = "/api/user/"
