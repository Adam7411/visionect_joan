...
import qrcode
import calendar
import json  # FIX: required by create_todo_list_url
...
# Local constants (moved from old __init__.py for brevity)
THEMED_FONTS = {
...
async def _add_interactive_layer_to_url(
    ...
    """
    if not (add_back_button or click_anywhere or click_anywhere_to_action or action_webhook_id or action_webhook_2_id or back_url):
        return data_url
    ...
    try:
        ...
        padding_css = "padding-bottom: 120px !important;" if buttons_visible else ""
        ...
        *, *:before, *:after {{ box-sizing: inherit; }}
        ...
        .visionect-interactive-button:active {{
            border-color: #000;
            opacity: 0.7;
        }}
        ...
        add_back_button_effective = add_back_button

        buttons_html = ""
        if buttons_visible:
            bar_content = ""
            if add_back_button_effective and back_url:
                bar_content += f'<a href="{back_url}" class="visionect-interactive-button icon-back" onclick="if(event)event.stopPropagation();"></a>'
            
            if webhook_url_2:
                bar_content += f'<div class="visionect-interactive-button icon-action2" onclick="triggerWebhook(event, \'{webhook_url_2}\')"></div>'

            if webhook_url_1:
                bar_content += f'<div class="visionect-interactive-button icon-action1" onclick="triggerWebhook(event, \'{webhook_url_1}\')"></div>'
            
            if bar_content:
                buttons_html = f'<div class="visionect-button-bar">{bar_content}</div>'

        if buttons_html:
            decoded_html = re.sub(r"(<body\b[^>]*>)", r"\1" + buttons_html, decoded_html, count=1, flags=re.IGNORECASE)
        
        interactive_scripts = ""
        if webhook_url_1 or webhook_url_2:
            interactive_scripts += (
                "function triggerWebhook(event, url){"
                " try{ if(event){event.stopPropagation();}"
                " fetch(url,{method:'POST',body:'{}'}).catch(function(err){console.error('Webhook failed:',err);});"
        ...
        if click_anywhere_to_action and webhook_url_1:
            ...
            overlay_html = f'<div id="visionect-click-overlay" onclick="try{{window.location.href=\'{back_url}\';}}catch(e){{}}"></div>'
    ...
    for entity_id in entity_ids:
        ...
        try:
            ...
            if float(val).is_integer():
                ...
                fmt_val = f"{val:.1f}"  # jedna cyfra po przecinku
            display_state_val = f"{fmt_val} {unit}" if unit else fmt_val
        except (ValueError, TypeError):
            # Nie liczba – użyj tłumaczeń stanów tekstowych
...
async def create_todo_list_url(
    ...
    style_css = f"""
        ...
        /* Zapobieganie zaznaczaniu tekstu przy kliknięciu */
    ...
    # Zbuduj URL webhooka (jeśli podano ID)
    ...
    if not items:
        ...
        # Sortuj tak, by aktywne były wyżej
        ...
        for item in sorted_items:
            ...
            # onclick wywołuje toggleItem – jeśli nie ma webhooka, działa tylko wizualnie
    ...
    # Skrypt do lokalnego przełączania (UI) i opcjonalnie wysyłki do webhooka
    script_js = f"""
        <script>
...
async def create_rss_feed_url(hass, title: str, items: list, lang: str, small_screen: bool) -> str:
    ...
    per_page = 7 if small_screen else 10  # ile pozycji na stronie
    ...
    style_css = f"""
        ...
        .content {{
            ...
            /* brak scrolla – pokazujemy jedną stronę naraz */
            overflow: hidden;
            position: relative;
        }}
        ...
        /* Zapobieganie zaznaczaniu tekstu na e-ink */
    ...
    # Zbuduj listę <li> i rozbij na strony po per_page
    ...
    # Zbuduj HTML dla stron – pierwsza aktywna
    pages_html = []
...
async def create_weather_url(
    ...
    style_css = f"""
        ...
        /* POWIĘKSZENIE GŁÓWNEJ IKONY: było 180/220px, teraz 220/300px */
    ...
    # NEW: utility to get temperature unit from attributes
    temp_unit = attrs.get('temperature_unit') or "°C"

    if layout == "weather_graph_panel":
        # Generate weather graph from hourly forecast
        graph_bytes = _generate_weather_forecast_graph(hourly_forecast or [], lang, temp_unit, is_portrait)
        if graph_bytes:
        ...
        # Current conditions strip below the graph
        ...
        # List-only daily forecast over the whole screen
        items_html = ""
        if not daily_forecast:
            ...
            for day in daily_forecast[:7]:
                ...
                if 'precipitation' in day or 'precipitation_probability' in day:
                    ...
                    # Optionally show probability if present
...
def _generate_graph_image(
    ...
    # Rysujemy bez constrained_layout, aby móc ręcznie sterować górnym marginesem
