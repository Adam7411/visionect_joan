...
THEMED_FONTS = {
    ...
    "Bungee Shade": "'B bungee Shade', cursive",
...
async def _add_interactive_layer_to_url(
    ...
    try:
        ...
        # Pasek i padding są widoczne tylko, gdy:
        # 1. Tryb "click anywhere" jest WYŁĄCZONY.
        # 2. Jest DEFINIOWANY co najmniej jeden przycisk (wstecz lub webhook).
        ...
        *, *:before, *:after {{ box-sizing: inherit; }}
        ...
        .button-area {{
            flex: 1; /* Each takes up 1/3 of the width */
            height: 100%;
            display: flex;
        ...
        buttons_html = ""
        # Renderuj pasek tylko, jeśli jest to potrzebne
...
async def create_status_panel_url(hass, title: str, entity_ids: list[str], lang: str, orientation: str, small_screen: bool) -> str:
    ...
    cache_buster_comment = f'<!-- cb:{int(time.time())} -->'  # <--- MODYFIKACJA
    html_content = f'<!DOCTYPE html><html><head><meta charset="UTF-8"><style>{style_css}</style></head><body>{html_body}{cache_buster_comment}</body></html>' # <--- MODYFIKACJA
...
def create_text_message_url(message: str, text_color: str = "black", background_color: str = "white", text_size: str = "28px", text_align: str = "center", font_family_key: str = "Modern", font_weight: str = "normal", layout: str = "text_only", image_url: str = None, image_zoom: int = 100, image_rotation: int = 0) -> str:
    ...
    cache_buster_comment = f'<!-- cb:{int(time.time())} -->' # <--- MODYFIKACJA
    html_content = f'<!DOCTYPE html><html><head><meta charset="UTF-8"><style>{style_css}</style></head><body>{body_html}{cache_buster_comment}</body></html>' # <--- MODYFIKACJA
    encoded = urllib.parse.quote(html_content, safe='')
    return f"data:text/html,{encoded}"
...
async def create_todo_list_url(
    ...
    cache_buster_comment = f'<!-- cb:{int(time.time())} -->' # <--- MODYFIKACJA
    html_content = f'<!DOCTYPE html><html><head><meta charset="UTF-8"><title>{html.escape(title)}</title><style>{style_css}</style></head><body>{html_body}{cache_buster_comment}</body></html>' # <--- MODYFIKACJA
    encoded = urllib.parse.quote(html_content, safe='')
    return f"data:text/html,{encoded}"
...
async def create_energy_panel_url(hass, states: dict[str], orientation: str, lang: str, small_screen: bool) -> str:
    ...
    cache_buster_comment = f'<!-- cb:{int(time.time())} -->' # <--- MODYFIKACJA
    html_content = f'<!DOCTYPE html><html><head><meta charset="UTF-8"><title>{panel_title}</title><style>{style_css}</style></head><body>{html_body}{cache_buster_comment}</body></html>' # <--- MODYFIKACJA
    encoded = urllib.parse.quote(html_content, safe='')
    return f"data:text/html,{encoded}"
...
def create_calendar_url(events: list, style: str = "modern", lang: str = "en") -> str:
    ...
    cache_buster_comment = f'<!-- cb:{int(time.time())} -->' # <--- MODYFIKACJA
    html_content = f'<!DOCTYPE html><html><head><meta charset="UTF-8"><title>{calendar_title}</title><style>{style_css}</style></head><body>{html_body_content}{cache_buster_comment}</body></html>' # <--- MODYFIKACJA
    encoded = urllib.parse.quote(html_content, safe='')
    return f"data:text/html,{encoded}"
...
def create_monthly_calendar_url(year: int, month: int, events: list, lang: str = "en", small_screen: bool = False) -> str:
    ...
    cache_buster_comment = f'<!-- cb:{int(time.time())} -->' # <--- MODYFIKACJA
    html_content = f'<!DOCTYPE html><html><head><meta charset="UTF-8"><title>{calendar_title}</title><style>{style_css}</style><script>{js_script}</script></head><body>{monthly_view_html}{daily_views_html}{cache_buster_comment}</body></html>' # <--- MODYFIKACJA
    encoded = urllib.parse.quote(html_content, safe='')
    return f"data:text/html,{encoded}"
...
async def create_weather_url(
    ...
    cache_buster_comment = f'<!-- cb:{int(time.time())} -->' # <--- MODYFIKACJA
    html_content = f'<!DOCTYPE html><html><head><meta charset="UTF-8"><title>{weather_title}</title><style>{style_css}</style></head><body>{html_body}{cache_buster_comment}</body></html>' # <--- MODYFIKACJA
    encoded = urllib.parse.quote(html_content, safe='')
    return f"data:text/html,{encoded}"
