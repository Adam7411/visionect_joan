...
async def async_get_icon_as_base64(hass, icon_name: str) -> str:
    """Load icon for buttons/panels.
    Checks in this order:
    1. svg_button/<icon_name>
    2. svg/<icon_name>
    3. svg/default.svg (fallback)
    """
    if not icon_name:
        return ""
    ...
    if not icon_file:
        fallback_file = base_dir / "svg" / "default.svg"
        if fallback_file.is_file():
            _LOGGER.warning(
                "Icon '%s' not found in svg_button/ or svg/, using default.svg", icon_name
            )
            icon_file = fallback_file
        else:
            _LOGGER.error("Icon '%s' not found and default.svg is also missing!", icon_name)
            return ""
    ...
    def _load_icon() -> str:
        try:
            ...
            _LOGGER.error("Error encoding icon %s: %s", icon_name, e)
            return ""
...
async def create_status_panel_url(hass, title: str, entity_ids: list[str], lang: str, orientation: str, small_screen: bool) -> str:
    ...
    header_html = f"""<div class="header">{html.escape(title)}</div>""" if (title or "").strip() else ""
    html_body = f""" {header_html} <div class="grid">{items_html}</div> """
...
async def create_rss_feed_url(hass, title: str, items: list, lang: str, small_screen: bool) -> str:
    ...
    prev_disabled = " disabled" if total_pages <= 1 else ""
...
""".replace("__MAX__", str(total_pages))
    ...
    <div class="pager">
        <div id="btn-prev" class="pager-btn{prev_disabled}" onclick="goPrev()">&larr; {btn_prev}</div>
        <div class="pager-info">{page_word} <span id="page-cur">1</span>/<span id="page-max">{total_pages}</span></div>
        <div id="btn-next" class="pager-btn{next_disabled}" onclick="goNext()">{btn_next} &rarr;</div>
    </div>
...
async def create_energy_panel_url(hass, states: dict[str, Any], orientation: str, lang: str, small_screen: bool) -> str:
...
def create_calendar_list_view_html(events: list, style: str = "modern", add_back_button: bool = False, day_date: datetime.date = None, lang: str = "en") -> str:
    ...
    day_html_blocks = []
    for event_date, day_events in sorted(events_by_day.items()):
...
def create_monthly_calendar_url(year: int, month: int, events: list, lang: str = "en", small_screen: bool = False) -> str:
    ...
    style_css = f"""
        ...
        .no-events {{ text-align: center; font-size: 2em; }}
    """
    js_script = """
...
def _generate_weather_forecast_graph(
    ...
    try:
        ...
        for forecast in hourly_forecast[:25]:
            if isinstance(forecast, dict) and 'datetime' in forecast and 'temperature' in forecast:
                ts = dt_util.parse_datetime(forecast['datetime'])
                if ts is None:
                    continue
...
async def create_weather_url(
    ...
    items_count = min(len(daily_forecast or []), 7) if layout == "daily_forecast_list" else 0
    if layout == "daily_forecast_list":
        if small_screen:
            if items_count <= 4:
                daily_item_fs = "3.2em"; daily_icon_px = "100px"; daily_item_pad = "18px 0"
                day_w = "170px"; precip_w = "170px"; drop_px = "40px"; gap = "10px"
            elif items_count <= 6:
                daily_item_fs = "2.6em"; daily_icon_px = "85px"; daily_item_pad = "14px 0"
                day_w = "160px"; precip_w = "160px"; drop_px = "36px"; gap = "8px"
            else:
                daily_item_fs = "2.3em"; daily_icon_px = "72px"; daily_item_pad = "12px 0"
                day_w = "150px"; precip_w = "150px"; drop_px = "32px"; gap = "6px"
        else:
            if items_count <= 5:
                daily_item_fs = "4.0em"; daily_icon_px = "120px"; daily_item_pad = "24px 0"
                day_w = "200px"; precip_w = "200px"; drop_px = "48px"; gap = "14px"
            elif items_count <= 7:
                daily_item_fs = "3.4em"; daily_icon_px = "100px"; daily_item_pad = "18px 0"
                day_w = "180px"; precip_w = "180px"; drop_px = "44px"; gap = "12px"
            else:
                daily_item_fs = "3.1em"; daily_icon_px = "90px"; daily_item_pad = "16px 0"
                day_w = "170px"; precip_w = "170px"; drop_px = "40px"; gap = "10px"
    ...
    if layout == "daily_forecast_list":
        style_css = f"""
            ...
            .daily-list-standalone {{ padding: 10px 10px 0 10px; box-sizing: border-box; display: flex; flex-direction: column; justify-content: flex-start; gap: {gap}; flex-grow: 1; }}
            .daily-list-standalone .daily-item {{ font-size: {daily_item_fs}; padding: {daily_item_pad}; display: flex; align-items: center; border-bottom: 2px solid #ccc; }}
            .daily-list-standalone .daily-item:last-child {{ border-bottom: none; }}
            .daily-list-standalone .daily-item .day {{ flex: 0 0 {day_w}; font-weight: bold; }}
            .daily-list-standalone .daily-item .icon {{ width: {daily_icon_px}; height: {daily_icon_px}; margin: 0 18px; }}
            .daily-list-standalone .daily-item .temps {{ flex-grow: 1; text-align: center; }}
            .daily-list-standalone .daily-item .precip {{ display: flex; align-items: center; justify-content: flex-end; flex: 0 0 {precip_w}; }}
            .daily-list-standalone .daily-item .precip .icon {{ width: {drop_px}; height: {drop_px}; margin-right: 10px; }}
        """
    else:
        style_css = f"""
...
def create_simple_cache_buster(url: str) -> str:
    ...
    if url.startswith("data:text/html"):
        try:
            ...
            dec = urllib.parse.unquote(payload)
            dec = re.sub(r"</body>\s*</html>\s*$", f"<!-- cb:{int(time.time())} --></body></html>", dec, flags=re.IGNORECASE)
            return f"{header},{urllib.parse.quote(dec, safe='')}"
        except Exception:
            return url
...
async def create_button_panel_url(hass, title: str, buttons: list[dict], small_screen: bool) -> str:
    ...
    style_css = f"""
        ...
        .button-item:active {{ background-color: #ccc; }}
        .button-item .icon {{ width: {icon_size}; height: {icon_size}; margin-bottom: 8px; }}
        .button-item .name {{ font-size: {button_name_fs}; font-weight: bold; line-height: 1.2; }}
        .button-item .sub-name {{ font-size: {sub_name_fs}; opacity: 0.8; margin-top: 5px; line-height: 1.1; }}
    """
