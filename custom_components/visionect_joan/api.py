...
class VisionectAPI:
    ...
    async def async_set_device_url(
        ...
        *,
        ...
        url_exact = str(url or "").strip()
        if recent:
            recent_url, recent_ts = recent
            recent_norm = self._normalize_url_for_battery_guard(recent_url)
            recent_exact = str(recent_url or "").strip()
            # Exact same string (incl. ?cb=) within min interval — always skip.
            # Without force, also treat same path (cb ignored) as duplicate.
            same_for_duplicate = (
                url_exact == recent_exact
                if force
                else url_norm == recent_norm
            )
            if same_for_duplicate and (now_m - recent_ts) < self._session_url_min_interval_s:
        ...
        # Eco: ignore ?cb= (same page → skip PUT). force/wake_tablet: ?cb= must
        # differ so WebKit reloads when content at the same path changed.
        already_set = (
            current_url == url_exact
            if force
            else self._normalize_url_for_battery_guard(current_url) == url_norm
        )
        if already_set:
