...
class VisionectAPI:
    ...
    def _normalize_endpoint_for_hmac(self, endpoint: str) -> str:
        """
        Visionect HMAC signing is sensitive to RequestPath. Many REST endpoints
        accept a trailing slash; normalize to include it to avoid signature mismatches.
        We only modify 'safe' REST paths:
          - start with /api/
        ...
        try:
            if not endpoint.startswith("/api/"):
                return endpoint
            if endpoint.startswith("/api/live/") or endpoint.startswith("/backend/"):
                return endpoint
            if "?" in endpoint:
                return endpoint
    ...
    def _execute_request(self, method, endpoint, silent=False, **kwargs):
        ...
        endpoint_for_url = endpoint

        # Apply HMAC auth if configured
        if self.authenticated_by == "api_key_hmac" and self.api_key and self.api_secret:
            # Normalize endpoint for HMAC where appropriate
            endpoint_for_url = self._normalize_endpoint_for_hmac(endpoint)
            ...
            hmac_headers = self._build_hmac_headers(
                method, endpoint_for_url, has_json=has_json, json_body=json_body
            )
            ...
            # BasicAuth or cookie-based requests can follow redirects normally (caller may override)
            pass

        url = f"{self.base_url}{endpoint_for_url}"
    ...
    async def async_get_orphans(self) -> dict[str, str]:
        ...
        Returns:
            Dict mapping UUID to error description
            Example: {"uuid-123": "session_timeout", "uuid-456": "webkit_crash"}
        """
        try:
            response = await self._request("get", API_ORPHANS, silent=True)
            if response and isinstance(response, dict):
                 # Zakładamy format {uuid: error_msg} lub lista?
                 # Zgodnie z planem: Dict mapping UUID to error description
                 return response
            elif response and isinstance(response, list):
                 # Jeśli lista stringów, mapujemy na generic error
