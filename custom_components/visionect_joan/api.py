...
class VisionectAPI:
    ...
    async def async_set_display_rotation(self, uuid: str, display_rotation: str) -> bool:
        ...
        try:
            ...
            _LOGGER.error("Invalid display rotation value for %s: %s", uuid, display_rotation)
            return False

        if rotation_value not in (0, 1, 2, 3):
            _LOGGER.error("Display rotation out of range for %s: %s", uuid, rotation_value)
            return False
    ...
    async def async_get_tclv_list(self, uuid: str) -> list[dict] | None:
        ...
        data = await self._request("get", API_TCLV_LIST.format(uuid=uuid))
    ...
    async def async_get_tclv_values(self, uuid: str, type_ids: list[int]) -> dict[int, str]:
        ...
        resp = await self._request("post", API_TCLV_PARAM.format(uuid=uuid), json=payload_with_value, silent=True)
        if resp is None:
            resp = await self._request("post", API_TCLV_PARAM.format(uuid=uuid), json=payload_no_value, silent=True)
        out: dict[int, str] = {}
        ...
        def _collect_rows(node, bucket: list[dict]):
            ...
            if isinstance(node, dict):
                ...
                if "type" in keys and "value" in keys:
                    bucket.append(node)
        ...
        for tid in missing:
            ...
            one_resp = await self._request("post", API_TCLV_PARAM.format(uuid=uuid), json=one_payload_with_value, silent=True)
            if one_resp is None:
                one_resp = await self._request("post", API_TCLV_PARAM.format(uuid=uuid), json=one_payload_no_value, silent=True)
            one_rows: list[dict] = []
    ...
    async def async_set_tclv_values(self, uuid: str, values_by_type: dict[int, str]) -> bool:
        ...
        resp = await self._request("post", API_TCLV_PARAM.format(uuid=uuid), json=payload)
