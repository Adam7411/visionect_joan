...
class VisionectAPI:
    ...
    # --- Helpers for HMAC + endpoint normalization ---
    @staticmethod
    def _has_file_extension(endpoint: str) -> bool:
        last = endpoint.rsplit("/", 1)[-1]
        return "." in last

    @staticmethod
    def _should_append_trailing_slash(endpoint: str) -> bool:
        """Heuristic: add trailing slash for REST-like endpoints (no file ext, under /api/), but not for live image/png, etc."""
        ...
        if endpoint.startswith("/api/live/"):
            return False
        if VisionectAPI._has_file_extension(endpoint):
            return False
        # Backend and JSON REST endpoints tolerate trailing slash; add it to match Visionect HMAC docs.
        return endpoint.startswith("/api/")

    @staticmethod
    def _toggle_trailing_slash(endpoint: str) -> str:
        if endpoint.endswith("/"):
            return endpoint[:-1]
        return endpoint + "/"

    def _normalize_endpoint_for_hmac(self, endpoint: str) -> str:
        """Return endpoint normalized to include trailing slash when recommended by Visionect docs."""
        if not endpoint.startswith("/"):
            endpoint = "/" + endpoint
        if self._should_append_trailing_slash(endpoint):
            endpoint = endpoint + "/"
        return endpoint
    ...
    def _build_hmac_headers(self, method: str, endpoint: str, has_json: bool = False, json_body: str = None) -> dict:
        ...
        # IMPORTANT: RequestPath must include trailing slash per Visionect guidance (where applicable)
        endpoint_norm = self._normalize_endpoint_for_hmac(endpoint)
        ...
        # Add Content-Sha256 header when sending JSON (optional but useful for clarity)
        if has_json and content_sha256:
            headers["Content-Sha256"] = content_sha256
        return headers

    def _perform_request_once(self, method, endpoint, headers, allow_redirects, **kwargs):
        """Execute a single HTTP request and return (ok, response, parsed_payload)."""
        url = f"{self.base_url}{endpoint}"
        response = self.session.request(method, url, timeout=15, headers=headers, allow_redirects=allow_redirects, **kwargs)
        # Raise for HTTP errors to allow status inspection
        response.raise_for_status()

        content_type = response.headers.get('Content-Type', '') or ''
        if 'application/json' in content_type:
            return True, response, response.json()
        if 'image/' in content_type:
            return True, response, response.content
        return True, response, response.text

    def _execute_request(self, method, endpoint, silent=False, **kwargs):
        """Executes a synchronous HTTP request with improved error handling, HMAC normalization and fallback."""
        headers = kwargs.pop("headers", {}) or {}
        retry_count = 0

        # Apply HMAC auth if configured
        use_hmac = (self.authenticated_by == "api_key_hmac" and self.api_key and self.api_secret)

        # Serialize JSON exactly once if present
        json_body = None
        has_json = False
        if "json" in kwargs:
            try:
        ...
        # Do not follow redirects for HMAC-signed requests (path change -> signature mismatch)
        allow_redirects = kwargs.get("allow_redirects", True)
        if use_hmac:
            allow_redirects = False

        def prepare_headers(ep: str) -> dict:
            if use_hmac:
                self.session.auth = None
                hmac_headers = self._build_hmac_headers(method, ep, has_json=has_json, json_body=json_body)
                return {**hmac_headers, **headers}
            return headers

        # Decide primary endpoint form
        primary_ep = self._normalize_endpoint_for_hmac(endpoint) if use_hmac else endpoint
        secondary_ep = self._toggle_trailing_slash(primary_ep) if use_hmac else None

        while retry_count < MAX_RETRY_ATTEMPTS:
            try:
                # Try primary form
                ok, resp, payload = self._perform_request_once(method, primary_ep, prepare_headers(primary_ep), allow_redirects, **kwargs)
                return payload
            except (ConnectionError, Timeout) as e:
                retry_count += 1
                if not silent:
                    _LOGGER.warning(f"{e.__class__.__name__} to {self.base_url}{primary_ep} (attempt {retry_count}/{MAX_RETRY_ATTEMPTS}): {e}")
                if retry_count >= MAX_RETRY_ATTEMPTS:
                    if not silent:
                        _LOGGER.error(f"Failed to connect to {self.base_url}{primary_ep} after {MAX_RETRY_ATTEMPTS} attempts")
                    return None
                sleep_s = min(2 ** (retry_count - 1), 60) + random.uniform(0, 1)
                time.sleep(sleep_s)
            except HTTPError as e:
                # If HMAC and auth/redirect issues -> try toggled trailing slash once
                status = e.response.status_code if e.response is not None else None
                is_auth_or_redirect = status in (401, 403, 301, 302, 307, 308)
                if use_hmac and secondary_ep and is_auth_or_redirect:
                    try:
                        ok2, resp2, payload2 = self._perform_request_once(method, secondary_ep, prepare_headers(secondary_ep), allow_redirects, **kwargs)
                        return payload2
                    except Exception as e2:
                        if not silent:
                            _LOGGER.error(f"HMAC fallback failed ({primary_ep} -> {secondary_ep}): {e2}")
                        return None
                if not silent:
                    _LOGGER.error(f"Request error to {self.base_url}{primary_ep}: {e}")
                return None
    ...
    async def async_set_display_rotation(self, uuid: str, display_rotation: str) -> bool:
        """Sets the device's screen rotation by sending a full payload."""
        device_data = await self._request("get", API_DEVICE_DETAIL.format(uuid=uuid))
    ...
    async def async_get_device_screenshot(self, uuid: str) -> bytes | None:
        ...
        # Do NOT add trailing slash here (file extension present)
