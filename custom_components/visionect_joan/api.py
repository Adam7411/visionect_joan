...
class VisionectAPI:
    ...
    def __init__(self, hass, api_url: str, username: str = None, password: str = None,
        ...
        self._orphans_cache = None
        self._orphans_cache_time = 0
        ...
        self._session_url_change_guard_s = 30.0
    ...
    def _device_guard_interval(self, uuid: str) -> float:
        ...
        nu = self._normalize_uuid(uuid)
        profile = self._device_refresh_profile.get(nu, "normal")
    ...
    async def async_set_device_url(self, uuid: str, url: str, guard_seconds_override: float | None = None) -> bool:
        normalized_uuid = self._normalize_uuid(uuid)
        ...
        if recent:
            ...
            if recent_url == url and (now_m - recent_ts) < self._session_url_min_interval_s:
        ...
        response = await self._request("put", API_SESSION_DETAIL.format(uuid=normalized_uuid), json=session_data)
        if response is None:
            return False
    ...
    async def async_set_display_rotation(self, uuid: str, display_rotation: str) -> bool:
        ...
        response = await self._request("put", API_DEVICE_DETAIL.format(uuid=nu), json=device_data)
    ...
    async def async_get_orphans(self) -> dict[str, str]:
        """Fetches list of problematic sessions/devices from /api/orphans endpoint with caching."""
        current_time = time.time()

        # Check cache
        if self._orphans_cache is not None and (current_time - self._orphans_cache_time) < self._orphans_cache_ttl:
            _LOGGER.debug("Using cached orphans list")
        ...
        try:
            ...
            if response and isinstance(response, dict):
                 self._orphans_cache = response
                 self._orphans_cache_time = current_time
                 return response
            elif response and isinstance(response, list):
                 orphans_dict = {uuid: "unknown_issue" for uuid in response}
                 self._orphans_cache = orphans_dict
                 self._orphans_cache_time = current_time
                 return orphans_dict
            return {}
        except Exception as e:
