...
from . import SERVICE_SEND_CALENDAR, SERVICE_SEND_WEATHER, SERVICE_SEND_TODO_LIST
...
async def async_setup_entry(
    ...
    # Pobieramy predefiniowane widoki z configuration.yaml
    views = hass.data[DOMAIN].get('views', [])
    # Pobieramy wszystkie dostępne w HA encje
    available_calendars = hass.states.async_entity_ids("calendar")
    available_weather = hass.states.async_entity_ids("weather")
    available_todos = hass.states.async_entity_ids("todo")

    # Dla każdego urządzenia Joan tworzymy komplet encji select
    for uuid in coordinator.data:
        # 1. Encja do wyboru predefiniowanych widoków
        entities.append(VisionectViewSelect(coordinator, uuid, views))

        # 2. Encja do wysyłania kalendarza
        if available_calendars:
            entities.append(VisionectCalendarSelect(coordinator, uuid, available_calendars))
        
        # 3. POPRAWKA: Przywrócona encja do wysyłania pogody
        if available_weather:
            entities.append(VisionectWeatherSelect(coordinator, uuid, available_weather))
            
        # 4. Encja do wysyłania listy zadań
        if available_todos:
            entities.append(VisionectTodoListSelect(coordinator, uuid, available_todos))
...
class VisionectCalendarSelect(VisionectEntity, SelectEntity):
    """Encja select do wysyłania kalendarza na urządzenie."""

    def __init__(self, coordinator, uuid: str, calendars: list[str]):
        super().__init__(coordinator, uuid)
        self._attr_translation_key = "calendar_select"
        self._attr_unique_id = f"{uuid}_calendar_select"
        # POPRAWKA: Upewniamy się, że ikona jest zdefiniowana
        self._attr_icon = "mdi:calendar-send"
        self._attr_options = calendars
        self._attr_current_option = None 

    async def async_select_option(self, option: str) -> None:
        """Wywołuje usługę wysyłania kalendarza."""
        _LOGGER.info(f"Żądanie wysłania kalendarza '{option}' na urządzenie {self.uuid}")
        
        device_registry_instance = dr.async_get(self.hass)
        device_entry = device_registry_instance.async_get_device(identifiers={(DOMAIN, self.uuid)})

        if not device_entry:
            _LOGGER.error(f"Nie znaleziono device_id dla UUID {self.uuid}")
            return

        await self.hass.services.async_call(
            DOMAIN,
            SERVICE_SEND_CALENDAR,
            {
                "device_id": device_entry.id,
                "calendar_entity": option,
                "display_style": "monthly_grid"
            },
            blocking=False,
        )
        self._attr_current_option = None
...
class VisionectWeatherSelect(VisionectEntity, SelectEntity):
    """Encja select do wysyłania pogody na urządzenie."""

    def __init__(self, coordinator, uuid: str, weather_entities: list[str]):
        super().__init__(coordinator, uuid)
        self._attr_translation_key = "weather_select"
        self._attr_unique_id = f"{uuid}_weather_select"
        self._attr_icon = "mdi:weather-partly-cloudy"
        self._attr_options = weather_entities
        self._attr_current_option = None 

    async def async_select_option(self, option: str) -> None:
        """Wywołuje usługę wysyłania pogody."""
        _LOGGER.info(f"Żądanie wysłania pogody '{option}' na urządzenie {self.uuid}")
...
class VisionectTodoListSelect(VisionectEntity, SelectEntity):
    """Encja select do wysyłania listy zadań na urządzenie."""

    def __init__(self, coordinator, uuid: str, todo_entities: list[str]):
        super().__init__(coordinator, uuid)
        self._attr_translation_key = "todo_list_select"
        self._attr_unique_id = f"{uuid}_todo_list_select"
        self._attr_icon = "mdi:format-list-checks"
        self._attr_options = todo_entities
        self._attr_current_option = None 

    async def async_select_option(self, option: str) -> None:
        """Wywołuje usługę wysyłania listy zadań."""
        _LOGGER.info(f"Żądanie wysłania listy zadań '{option}' na urządzenie {self.uuid}")
