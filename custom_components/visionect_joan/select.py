...
from .profile_tuning import apply_api_cache_ttls_for_profile_map
...
async def async_setup_entry(
    ...
    for uuid, device_coordinator in device_coordinators.items():
        entities.append(VisionectViewSelect(device_coordinator, uuid, views))
...
class VisionectScreenSizeSelect(VisionectEntity, SelectEntity):
    ...
    def _stored_size(self) -> str | None:
        prefs = self._get_prefs()
        raw = (prefs.get("screen_size_by_uuid") or {}).get(self.uuid)
        if raw is None:
            return None
    ...
    async def async_select_option(self, option: str) -> None:
        ...
        if option == "auto":
            prefs["screen_size_by_uuid"].pop(self.uuid, None)
        else:
            prefs["screen_size_by_uuid"][self.uuid] = option
        store = self._get_store()
        if store:
            await store.async_save(prefs)
        self.async_write_ha_state()
...
class VisionectRefreshProfileSelect(VisionectEntity, SelectEntity):
    ...
    def current_option(self) -> str | None:
        prefs = self._get_prefs()
        raw = (prefs.get("refresh_profile_by_uuid") or {}).get(self.uuid)
    ...
    async def async_select_option(self, option: str) -> None:
        if option not in REFRESH_PROFILE_SELECT_OPTIONS:
            ...
            return
        prefs = self._get_prefs()
        prefs.setdefault("refresh_profile_by_uuid", {})[self.uuid] = option
        store = self._get_store()
        if store:
            await store.async_save(prefs)
        api: VisionectAPI = self.hass.data[DOMAIN][self.coordinator.config_entry.entry_id]["api"]
        ...
        n_title, n_body = _refresh_profile_notification(self.hass, self.uuid, device_name, option)
        async_create(
            self.hass,
            n_body,
            title=n_title,
            notification_id=f"visionect_refresh_profile_{self.uuid}",
        )
        self.async_write_ha_state()
