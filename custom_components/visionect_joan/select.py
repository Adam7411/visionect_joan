...
# ---> NOWOŚĆ: Importujemy nazwy usług, których będziemy używać <---
from . import SERVICE_SEND_CALENDAR, SERVICE_SEND_WEATHER

_LOGGER = logging.getLogger(__name__)

INSTRUCTION_OPTION_KEY = "unconfigured_prompt"

# ---> ZMIANA: Cała funkcja `async_setup_entry` została przebudowana <---
async def async_setup_entry(
    ...
    # Pobieramy wszystkie dostępne w HA encje kalendarzy i pogody
    ...
    for uuid in coordinator.data:
        # 1. Encja do wyboru predefiniowanych widoków (bez zmian)
        entities.append(VisionectViewSelect(coordinator, uuid, views))

        # 2. Encja do wysyłania kalendarza (jeśli są dostępne kalendarze)
        if available_calendars:
            entities.append(VisionectCalendarSelect(coordinator, uuid, available_calendars))
        
        # 3. Encja do wysyłania pogody (jeśli są dostępne encje pogody)
