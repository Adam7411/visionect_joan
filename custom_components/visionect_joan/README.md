...
6. [Predefined Views](#predefined-views)  
   - [Configure (options menu)](#configure-options-menu)  
7. [Entities](#entities)  
...
3. Add `https://github.com/Adam7411/visionect_joan` as “Integration”.
4. Search “Visionect Joan” → Install.
5. Restart Home Assistant.

### Manual
1. Copy the `visionect_joan` folder from this repository into `/config/custom_components/` (or download the repo as ZIP and take `custom_components/visionect_joan/`).
2. Ensure the path is `/config/custom_components/visionect_joan/`.
...
**Settings → Devices & Services → Visionect Joan → Configure** opens a menu with:

| Area | Purpose |
|------|---------|
| **Predefined views** | Add, edit, or delete named URLs (same data as above). |
| **Recovery page** | Token for the Home Assistant–hosted recovery / probe URL; copy into VSS HTML Backend when needed. |
| **Cache cleanup** | Maximum age (hours) and run interval for deleting temporary files under `www` / `visionect_cache`. |
| **Tablet language** | Language for generated on-tablet HTML (`auto` follows Home Assistant). |
| **Battery alerts** | Low-battery threshold (5 / 10 / 15 %), on-tablet banner, Home Assistant notifications (low / full charge). |
| **Ollama** | Optional periodic VSS log analysis (URL, model, API key, interval in minutes). |
...
| `button` | Force Refresh / Reboot / Clear Web Cache / Check health / Ollama log analysis | Reboot & Clear cache disabled by default (diagnostic/config categories) |
...
| `visionect_joan.send_crypto` | Finance | Yes | CryptoCompare data + sparkline; coin IDs like CoinGecko slugs or tickers (no HA crypto integration) |
| `visionect_joan.set_session_options` | Render | N/A | Bit depth / dithering |
| `visionect_joan.read_safe_device_config` | Maintenance | N/A | Read safe TCLV values (persistent notification + `command_result` event) |
| `visionect_joan.apply_safe_device_config` | Maintenance | N/A | Write heart beat / system screens / touch mode (TCLV) |
| `visionect_joan.restore_safe_device_config` | Maintenance | N/A | Restore values backed up on last successful apply |
...
- `visionect_joan.send_crypto` – Live crypto panel: **CryptoCompare** supplies prices and history (no API key). Selectors use **CoinGecko-style** coin slugs for convenience; they map to tickers for the API. Optional back / auto-return like other content services.  
- `visionect_joan.read_safe_device_config` / `apply_safe_device_config` / `restore_safe_device_config` – **Safe Config (TCLV)**: read current safe parameters, apply heart beat / system screens / touch mode, or restore the backup created on the last apply. Typical flow: **read** → review notification → **apply**; use **restore** to roll back.  
...
### HAOS / Raspberry Pi — setup fails on `matplotlib`

If **matplotlib** appears in `manifest.json` as a pip requirement, Home Assistant tries to install it in the container. On some platforms (e.g. **Python 3.14**, **aarch64**, restricted build tools) that step can **fail** (Meson / wheel build errors) and **block the integration** from loading.

**In the current integration, matplotlib is not a declared requirement.** Home Assistant starts the integration normally; **sensor history graphs** and the **weather “graph” layout** skip PNG generation when matplotlib is not available (a short message may appear on the tablet for `send_sensor_graph`). If matplotlib is present in the environment, graphs work without extra configuration.

### Joan 13 Pro — static text turns gray; live values stay dark

> **Damage risk:** Official Visionect documentation warns that **wrong advanced display settings (including VCOM) can harm the e-paper panel**. Follow this **only** when **Visionect support** has told you these parameters apply to **your** Joan 13 Pro. **Never** copy the numbers to Joan 6, other Joan 13 variants, or other devices.

**Typical pattern:** Parts of the screen that **change rarely** start to look **gray or washed out** after roughly **1–2 minutes**, while **often-updated** areas (temperatures, entity states, counters) stay **black/clear**. That is usually **device-side e-paper tuning in VSS**, not Home Assistant, AppDaemon, or this integration.

**If support asked you to adjust the panel** in Visionect Secure Signage (VSS):

1. Select the device → **Advanced** → **Device Configuration**.
2. Open **Display** (top left).
3. For display **0**: set **VCOM** to **2450** and **Display Type / ID** to **3759210982** *(example pair cited by Visionect support for Joan 13 Pro — **your device may need different values**)*.
4. Click **Write to device** and wait until it completes.

More context: [Visionect System Boards](https://docs.visionect.com/TechnicalDocumentation/Legacy/VisionectProducts/VisionectSystemBoards.html)

<!-- Screenshot placeholder — add your image (e.g. VSS → Device Configuration → Display):
![VSS Display settings](docs/images/PLACEHOLDER-vss-display.png)
-->
