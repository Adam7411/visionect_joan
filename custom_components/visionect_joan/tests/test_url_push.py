...
def test_push_wake_tablet_forces_put_same_path() -> None:
    """wake_tablet must PUT even when only ?cb= differs (content refresh)."""
    import asyncio
    ...
    async def _run() -> None:
        ...
        result, skipped = await async_push_url_from_service(
            ...
            "dev-1",
            "http://ha/local/a.html?cb=99",
            {ATTR_WAKE_TABLET: True},
        )
        assert result == "success"
        assert skipped is False
        assert api.put_calls == [("dev-1", "http://ha/local/a.html?cb=99", True)]
...
def test_event_status_maps_skip_separately() -> None:
    from custom_components.visionect_joan.url_push import event_status_from_push_result

    assert event_status_from_push_result("success") == "success"
    assert event_status_from_push_result("skipped_unchanged") == "skipped"
    assert event_status_from_push_result("skipped_guard") == "skipped"
    assert event_status_from_push_result("failure") == "failure"
